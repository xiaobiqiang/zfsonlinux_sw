...
create_worker(tpool_t *tpool)
{
	...
	error = pthread_create(&tid, &tpool->tp_attr, tpool_worker, tpool);
	...
	pthread_attr_t *attr)
{
	...
	/* for linux, not supply like pthread_attr_clone function, 
		ignore attr parameter */
	error = pthread_attr_init(&tpool->tp_attr);
	if (error) {
		free(tpool);
		errno = error;
		return (NULL);
...
tpool_destroy(tpool_t *tpool)
{
	...
	pthread_mutex_lock(&tpool->tp_mutex);
	pthread_cleanup_push(pthread_mutex_unlock, &tpool->tp_mutex);
