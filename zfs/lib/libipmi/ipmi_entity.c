...
int
ipmi_entity_psu_status(ipmi_handle_t *ihp, ipmi_entity_t *ep,
	int *valp, int *isrstate)
{
	/* LINTED - alignment */
	ipmi_entity_impl_t *eip = ENTITY_TO_IMPL(ep);
	ipmi_entity_sdr_t *esp;
...
//	ipmi_sdr_fru_locator_t *frup;
//	char *frudata;
	boolean_t vender_plus, is_sbb = B_FALSE;
	int vender_step = 1;
	...
	for (esp = ipmi_list_next(&eip->ie_sdr_list); esp != NULL;
		...
		/*sbb */
		if(strcmp(esp->ies_name, "PS Status 1") == 0 ||
			strcmp(esp->ies_name, "PS Status 2") == 0)
		{
			vender_plus = B_TRUE;
			is_sbb = B_TRUE;
		}
		/* 4270 M2 */
		else if(strcmp(esp->ies_name, "PS0/PWROK") == 0 ||
			strcmp(esp->ies_name, "PS1/PWROK") == 0)
		{
			vender_plus = B_FALSE;
		}
		/* 4400 */
		else if(strcmp(esp->ies_name, "PS0/V_IN_ERR") == 0 ||
			strcmp(esp->ies_name, "PS1/V_IN_ERR") == 0)
		{
			vender_plus = B_TRUE;
		}
		/* 4270 M3 */
		else if(strcmp(esp->ies_name, "PS0/STATE") == 0 ||
			strcmp(esp->ies_name, "PS1/STATE") == 0)
		{
			vender_plus = B_TRUE;
			vender_step = 8;
		}
		...
		if ((srp = ipmi_get_sensor_reading(ihp, number)) == NULL) {
			if (ipmi_errno(ihp) == EIPMI_NOT_PRESENT) {
				*valp = B_FALSE;
				syslog(LOG_ERR, "entity_name is %s, not present.\n",
					esp->ies_name);
				return (0);
			}
			syslog(LOG_ERR, "entity_name is %s, get sensor state failed.\n",
				esp->ies_name);
			return (-1);
		}
		
		if (is_sbb) {
			*isrstate = srp->isr_state;
		}
		/* while isr_state != 1, then status is ok */
		if(vender_plus)
			*valp = ((srp->isr_state-vender_step) != 1);
		else
			*valp = (srp->isr_state != 1);

		return (0);

	}
	*valp = B_FALSE;
	syslog(LOG_ERR, "ep type is %d, not find any sensor state.\n",
			ep->ie_type);
	return (0);
}

int
ipmi_entity_fan_status(ipmi_handle_t *ihp, ipmi_entity_t *ep, boolean_t *valp)
{
	/* LINTED - alignment */
	ipmi_entity_impl_t *eip = ENTITY_TO_IMPL(ep);
//	ipmi_entity_impl_t *cp;
