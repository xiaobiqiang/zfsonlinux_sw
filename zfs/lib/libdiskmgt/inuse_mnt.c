...
inuse_mnt(char *slice, nvlist_t *attrs, int *errp)
{
	...
	*errp = 0;
	...
	(void) pthread_mutex_lock(&init_lock);
	if (!initialized) {
	    kthread_t	*mnttab_thread;
	    ...
	    *errp = load_mnttab(B_FALSE);
		...
		/* start a thread to monitor the mnttab */
			mnttab_thread = thread_create(NULL, 0, watch_mnttab, NULL,
...
#if 0
	int fd;
	if ((fd = open("/etc/mnttab", O_RDONLY)) != -1) {

	    char buf[81];

	    /* do the initial read so we don't get the event right away */
	    (void) read(fd, buf, (size_t)(sizeof (buf) - 1));
