...
/* LHL ADD 2014.02.16*/
	{ CARD_LINK, TOPO_STABILITY_PRIVATE },
	{ ETHERNET_LINK, TOPO_STABILITY_PRIVATE },
	{ FC_LINK, TOPO_STABILITY_PRIVATE },
	{ SAS_LINK, TOPO_STABILITY_PRIVATE },
	{ SAS_IPORT, TOPO_STABILITY_PRIVATE },
/* LHL ADD END */
	{ HEART_LINK, TOPO_STABILITY_PRIVATE },
	{ FANPSU, TOPO_STABILITY_PRIVATE },
	{ FAN, TOPO_STABILITY_PRIVATE },
	{ PSU, TOPO_STABILITY_PRIVATE }
...
fmri_nvl2str(nvlist_t *nvl, char *buf, size_t buflen)
{
	...
	if (nvlist_lookup_nvlist(nvl, FM_FMRI_HC_SPECIFIC, &hcsp) == 0) {
		...
		if (hcsn != NULL) {
			(void) snprintf(hexstr, sizeof (hexstr), "%lx", val);
			topo_fmristr_build(&size, buf, buflen, "/", NULL, NULL);
	...
	for (np = topo_child_first(pnp); np != NULL;
		...
		if (hc_match(mod, np, hwp->hcw_fmri, name, inst,
			...
			if (wp->tw_mod != NULL)
				...
				status = wp->tw_cb((topo_mod_t *)wp->tw_thp, np, hwp);
			topo_node_rele(np);
			wp->tw_node = node;
			return (status);
		}
