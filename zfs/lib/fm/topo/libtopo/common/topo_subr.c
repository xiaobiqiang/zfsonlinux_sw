...
#define	TOPO_PLATFORM_PATH	"/usr/platform/i86pc/lib/fm/topo/%s"
#define	TOPO_COMMON_PATH	"/usr/local/lib/topo/%s"
...
topo_search_path(topo_mod_t *mod, const char *rootdir, const char *file)
{
	 ...
	 * topo directories
	 */
	(void) snprintf(sp, PATH_MAX, TOPO_PLATFORM_PATH, file);
	if (access(sp, F_OK) != 0) {
 		(void) snprintf(sp, PATH_MAX, TOPO_PLATFORM_PATH, file);
		if (access(sp, F_OK) != 0) {
			(void) snprintf(sp, PATH_MAX, TOPO_COMMON_PATH, file);
			if (access(sp, F_OK) != 0) {
				return (NULL);
			}
		}
...
#define	TOPO_COMMON_PATH	"%s/usr/local/lib/topo/%s"
