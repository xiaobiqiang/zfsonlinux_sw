...
load_modepages(ds_scsi_info_t *sip)
{
	...
	if (result == 0 && datalength >= 0) {
		 ...
		 */
		if (datalength < 0)
			ddprintf("command returned invalid data length (%d)\n",
			    datalength);
		else
			ddprintf("failed to load modepages (KEY=0x%x "
			    "ASC=0x%x ASCQ=0x%x)\n", skey, asc, ascq);
...
verify_logpage(ds_scsi_info_t *sip, logpage_validation_entry_t *lp)
{
	...
	if (result == 0) {
		...
		ddprintf("failed to load %s log page (KEY=0x%x "
...
load_logpages(ds_scsi_info_t *sip)
{
	...
	if (result == 0) {
		...
		ddprintf("failed to get log pages "
    ...
    int log_length, nvlist_t *nvl)
{
	...
	if (!seen) {
		...
		ddprintf("IE logpage validation failed\n");
	}
    ...
    scsi_log_parameter_header_t *lphp, int log_length, nvlist_t *nvl)
{
	...
	if (bad_length || !has_reftemp) {
		...
		ddprintf("temperature logpage validation failed\n");
	}
    ...
    scsi_log_parameter_header_t *lphp, int log_length, nvlist_t *nvl)
{
	...
	if (bad) {
		...
		ddprintf("selftest logpage validation failed\n");
	}
...
load_ie_modepage(ds_scsi_info_t *sip)
{
	...
	if (result != 0) {
		ddprintf("failed to get IEC modepage (KEY=0x%x "
		...
		*changed = B_FALSE;
		...
		if (scsi_mode_select(sip,
			...
			*changed = B_TRUE;
			...
			ddprintf("failed to enable IE (KEY=0x%x "
			    "ASC=0x%x ASCQ=0x%x)\n", skey, asc, ascq);
			*changed = B_FALSE;
		}
	}
...
clear_gltsd(ds_scsi_info_t *sip)
{
	...
	if (result != 0) {
		ddprintf("failed to read Control mode page (KEY=0x%x "
		    "ASC=0x%x ASCQ=0x%x)\n", skey, asc, ascq);
	} else if (control_pg_cur.mode_page.length !=
	    PAGELENGTH_MODE_CONTROL_SCSI3) {
		ddprintf("SCSI-3 control mode page not supported\n");
	    ...
	    != 0) {
		ddprintf("failed to read changeable Control mode page (KEY=0x%x "
		    "ASC=0x%x ASCQ=0x%x)\n", skey, asc, ascq);
	} else if (control_pg_cur.gltsd && !GLTSD_CHANGEABLE(control_pg_chg)) {
		ddprintf("gltsd is set and not changeable\n");
		...
		result = scsi_mode_select(sip,
		    ...
		    MODEPAGE_CTRL_MODE_LEN, &hdrs, &skey, &asc, &ascq);
		if (result != 0)
			ddprintf("failed to enable GLTSD (KEY=0x%x "
			    "ASC=0x%x ASCQ=0x%x\n", skey, asc, ascq);
		if (nvlist_add_boolean_value(sip->si_dsp->ds_state,
		    "gltsd", control_pg_cur.gltsd) != 0)
			return (scsi_set_errno(sip, EDS_NOMEM));
    ...
    int log_length)
{
	...
	if (reftemp < DEFAULT_TEMPRATURE)
		reftemp = DEFAULT_TEMPRATURE;
...
analyze_ie_sense(ds_scsi_info_t *sip)
{
	...
	if (scsi_request_sense(sip, &skey, &asc, &ascq) != 0) {
		ddprintf("failed to request IE page (KEY=0x%x ASC=0x%x "
		    "ASCQ=0x%x)\n", skey, asc, ascq);
