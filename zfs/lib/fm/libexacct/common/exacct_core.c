...
{
	ea_item_t *item = &obj->eo_u.eo_u_item;
	...
	case EXT_DOUBLE:
		...
		break;
	case EXT_STRING:
		if ((item->ei_u.ei_u_string = ea_strdup((char *)value)) == NULL) {
			/* exacct_errno set above. */
			return (-1);
		}
		item->ei_size = strlen(item->ei_u.ei_u_string) + 1;
		break;
	case EXT_EXACCT_OBJECT:
		if ((item->ei_u.ei_u_object = ea_alloc(valsize)) == NULL) {
			/* exacct_errno set above. */
			return (-1);
		}
		bcopy(value, item->ei_u.ei_u_object, valsize);
		item->ei_size = valsize;
		break;
	case EXT_RAW:
		if ((item->ei_u.ei_u_raw = ea_alloc(valsize)) == NULL) {
			/* exacct_errno set above. */
			return (-1);
		}
		bcopy(value, item->ei_u.ei_u_raw, valsize);
		item->ei_size = valsize;
		break;
	default:
		exacct_seterr(EXR_INVALID_OBJ);
...
ea_free_object(ea_object_t *obj, int flag)
{
	...
	while (next != NULL) {
		if (next->eo_type == EO_GROUP) {
			ea_free_object(next->eo_u.eo_u_group.eg_objs, flag);
		} else if (next->eo_type == EO_ITEM) {
			switch (next->eo_catalog & EXT_TYPE_MASK) {
			case EXT_STRING:
				if (flag == EUP_ALLOC)
					ea_strfree(next->eo_u.eo_u_item.ei_u.ei_u_string);
				break;
			case EXT_RAW:
			case EXT_EXACCT_OBJECT:
				if (flag == EUP_ALLOC)
					ea_free(next->eo_u.eo_u_item.ei_u.ei_u_raw,
					    next->eo_u.eo_u_item.ei_size);
				break;
			default:
...
ea_free_item(ea_object_t *obj, int flag)
{
	...
	case EXT_STRING:
		if (flag == EUP_ALLOC)
			ea_strfree(obj->eo_u.eo_u_item.ei_u.ei_u_string);
		break;
	case EXT_RAW:
	case EXT_EXACCT_OBJECT:
		if (flag == EUP_ALLOC)
			ea_free(obj->eo_u.eo_u_item.ei_u.ei_u_raw, obj->eo_u.eo_u_item.ei_size);
		break;
	default:
		/* No action required for other types. */
		break;
...
ea_attach_to_group(ea_object_t *group, ea_object_t *obj)
{
	...
	group->eo_u.eo_u_group.eg_nobjs += n;

	for (nextp = &group->eo_u.eo_u_group.eg_objs; *nextp != NULL;
	    nextp = &(*nextp)->eo_next)
		continue;
...
ea_pack_object(ea_object_t *obj, void *buf, size_t bufsize)
{
	...
	for (;;) {
		...
		if (curr_obj->eo_type == EO_GROUP) {
			 ...
			 */
			exacct_order32(&curr_obj->eo_u.eo_u_group.eg_nobjs);
			ea_cond_memcpy_at_offset(buf, curr_pos, bufsize,
			    &curr_obj->eo_u.eo_u_group.eg_nobjs,
			    sizeof (uint32_t));
			exacct_order32(&curr_obj->eo_u.eo_u_group.eg_nobjs);
			...
			estack[curr_frame].esf_offset = curr_pos;
			if ((curr_obj = curr_obj->eo_u.eo_u_group.eg_objs) == NULL) {
				end_of_group = 1;
			}
		...
		switch (curr_obj->eo_catalog & EXT_TYPE_MASK) {
		case EXT_UINT8:
			src = &curr_obj->eo_u.eo_u_item.ei_u.ei_u_uint8;
			size = sizeof (uint8_t);
			break;
		case EXT_UINT16:
			src = &curr_obj->eo_u.eo_u_item.ei_u.ei_u_uint16;
			size = sizeof (uint16_t);
			exacct_order16(src);
			break;
		case EXT_UINT32:
			src = &curr_obj->eo_u.eo_u_item.ei_u.ei_u_uint32;
			size = sizeof (uint32_t);
			exacct_order32(src);
			break;
		case EXT_UINT64:
			src = &curr_obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			size = sizeof (uint64_t);
			exacct_order64(src);
			break;
		case EXT_DOUBLE:
			src = &curr_obj->eo_u.eo_u_item.ei_u.ei_u_double;
			size = sizeof (double);
			exacct_order64((uint64_t *)src);
			break;
		case EXT_STRING:
			src = curr_obj->eo_u.eo_u_item.ei_u.ei_u_string;
			size = curr_obj->eo_u.eo_u_item.ei_size;
			break;
		case EXT_EXACCT_OBJECT:
			src = curr_obj->eo_u.eo_u_item.ei_u.ei_u_object;
			size = curr_obj->eo_u.eo_u_item.ei_size;
			break;
		case EXT_RAW:
			src = curr_obj->eo_u.eo_u_item.ei_u.ei_u_raw;
			size = curr_obj->eo_u.eo_u_item.ei_size;
			break;
		case EXT_NONE:
		default:
