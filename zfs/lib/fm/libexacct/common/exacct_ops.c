...
static int stack_check(ea_file_impl_t *f)
{
	if (f->ef_depth == NULL) {
		if ((f->ef_depth =
		    (ea_file_depth_t *)ea_alloc(DEFAULT_ENTRIES * sizeof (ea_file_depth_t)))
		    == NULL) {
	 ...
	 */
		...
		while (f->ef_ndeep >= 0 &&
			...
			if (xread(f, &scratch32, sizeof (scratch32)) !=
			    sizeof (scratch32)) {
				exacct_seterr(EXR_CORRUPT_FILE);
				return (-1);
			}
		}
		return (0);
...
exacct_seterr(int errval)
{
	...
	(void) pthread_setspecific(errkey, (void *)(intptr_t)errval);
}
...
ea_next_object(ea_file_t *ef, ea_object_t *obj)
{
	...
	if (f->ef_advance != 0) {
		if (fseeko(f->ef_fp, (off_t)f->ef_advance, SEEK_CUR) == -1) {
			exacct_seterr(EXR_SYSCALL_FAIL);
			return (EO_ERROR);
		}
    ...
    int flags)
{
	...
	if ((obj->eo_catalog & EXT_TYPE_MASK) == EXT_GROUP) {
		...
		exacct_order64(&sz);
		if (xread(f, &obj->eo_u.eo_u_group.eg_nobjs, sizeof (uint32_t)) !=
		    sizeof (uint32_t)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		exacct_order32(&obj->eo_u.eo_u_group.eg_nobjs);
		...
		if (stack_new_group(f, obj->eo_u.eo_u_group.eg_nobjs) != 0) {
			/* exacct_error set above */
			return (EO_ERROR);
		}
		 ...
		 */
		if (obj->eo_u.eo_u_group.eg_nobjs == 0) {
	...
	case EXT_RAW:
		...
		obj->eo_u.eo_u_item.ei_u.ei_u_string = buf;
		/*
		 * Maintain our consistent convention that string lengths
		 * include the terminating NULL character.
		 */
		obj->eo_u.eo_u_item.ei_size = sz;
		break;
	case EXT_UINT8:
		if (xread(f, &obj->eo_u.eo_u_item.ei_u.ei_u_uint8, sizeof (uint8_t)) !=
		    sizeof (uint8_t)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		obj->eo_u.eo_u_item.ei_size = sizeof (uint8_t);
		break;
	case EXT_UINT16:
		if (xread(f, &obj->eo_u.eo_u_item.ei_u.ei_u_uint16, sizeof (uint16_t)) !=
		    sizeof (uint16_t)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		exacct_order16(&obj->eo_u.eo_u_item.ei_u.ei_u_uint16);
		obj->eo_u.eo_u_item.ei_size = sizeof (uint16_t);
		break;
	case EXT_UINT32:
		if (xread(f, &obj->eo_u.eo_u_item.ei_u.ei_u_uint32, sizeof (uint32_t)) !=
		    sizeof (uint32_t)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		exacct_order32(&obj->eo_u.eo_u_item.ei_u.ei_u_uint32);
		obj->eo_u.eo_u_item.ei_size = sizeof (uint32_t);
		break;
	case EXT_UINT64:
		if (xread(f, &obj->eo_u.eo_u_item.ei_u.ei_u_uint64, sizeof (uint64_t)) !=
		    sizeof (uint64_t)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		exacct_order64(&obj->eo_u.eo_u_item.ei_u.ei_u_uint64);
		obj->eo_u.eo_u_item.ei_size = sizeof (uint64_t);
		break;
	case EXT_DOUBLE:
		if (xread(f, &obj->eo_u.eo_u_item.ei_u.ei_u_double, sizeof (double)) !=
		    sizeof (double)) {
			exacct_seterr(EXR_CORRUPT_FILE);
			return (EO_ERROR);
		}
		exacct_order64((uint64_t *)&obj->eo_u.eo_u_item.ei_u.ei_u_double);
		obj->eo_u.eo_u_item.ei_size = sizeof (double);
		break;
	default:
		/*
...
{
	ea_object_t *obj;
	uint_t nobjs = grp->eo_u.eo_u_group.eg_nobjs;
	int i;
	 ...
	 */
	grp->eo_u.eo_u_group.eg_nobjs = 0;
	grp->eo_u.eo_u_group.eg_objs = NULL;
	...
	if (nobjs != grp->eo_u.eo_u_group.eg_nobjs) {
		exacct_seterr(EXR_CORRUPT_FILE);
		return (-1);
	}
	exacct_seterr(EXR_OK);
...
validate_header(ea_file_t *ef, const char *creator)
{
	...
	for (n = 0; n < hdr_grp.eo_u.eo_u_group.eg_nobjs; n++) {
		...
		case EXT_UINT32 | EXC_DEFAULT | EXD_VERSION:
			if (scratch_obj.eo_u.eo_u_item.ei_u.ei_u_uint32 != EXACCT_VERSION) {
		...
		case EXT_STRING | EXC_DEFAULT | EXD_FILETYPE:
			if (strcmp(scratch_obj.eo_u.eo_u_item.ei_u.ei_u_string,
			    EXACCT_HDR_STR) != 0) {
				error = EXR_CORRUPT_FILE;
				goto error_case;
			}
		...
		case EXT_STRING | EXC_DEFAULT | EXD_CREATOR:
			f->ef_creator =
			    ea_strdup(scratch_obj.eo_u.eo_u_item.ei_u.ei_u_string);
...
ea_copy_object(const ea_object_t *src)
{
	...
	switch (src->eo_type) {
	case EO_GROUP:
		dst->eo_u.eo_u_group.eg_nobjs = 0;
		dst->eo_u.eo_u_group.eg_objs = NULL;
		break;
	case EO_ITEM:
		/* Items containing pointers need special treatment. */
		switch (src->eo_catalog & EXT_TYPE_MASK) {
		case EXT_STRING:
			if (src->eo_u.eo_u_item.ei_u.ei_u_string != NULL) {
				dst->eo_u.eo_u_item.ei_u.ei_u_string =
				    ea_strdup(src->eo_u.eo_u_item.ei_u.ei_u_string);
				if (dst->eo_u.eo_u_item.ei_u.ei_u_string == NULL) {
			...
			break;
		case EXT_RAW:
			if (src->eo_u.eo_u_item.ei_u.ei_u_raw != NULL) {
				dst->eo_u.eo_u_item.ei_u.ei_u_raw =
				    ea_alloc(src->eo_u.eo_u_item.ei_size);
				if (dst->eo_u.eo_u_item.ei_u.ei_u_raw == NULL) {
					ea_free_object(dst, EUP_ALLOC);
					return (NULL);
				}
				bcopy(src->eo_u.eo_u_item.ei_u.ei_u_raw, dst->eo_u.eo_u_item.ei_u.ei_u_raw,
				    (size_t)src->eo_u.eo_u_item.ei_size);
			}
			break;
		case EXT_EXACCT_OBJECT:
			if (src->eo_u.eo_u_item.ei_u.ei_u_object != NULL) {
				dst->eo_u.eo_u_item.ei_u.ei_u_object =
				    ea_alloc(src->eo_u.eo_u_item.ei_size);
				if (dst->eo_u.eo_u_item.ei_u.ei_u_object == NULL) {
					ea_free_object(dst, EUP_ALLOC);
					return (NULL);
				}
				bcopy(src->eo_u.eo_u_item.ei_u.ei_u_raw, dst->eo_u.eo_u_item.ei_u.ei_u_raw,
...
ea_copy_object_tree(const ea_object_t *src)
{
	...
	for (ret_obj = last = NULL; src != NULL;
		...
		if (src->eo_type == EO_GROUP) {
			dst->eo_u.eo_u_group.eg_objs =
			    ea_copy_object_tree(src->eo_u.eo_u_group.eg_objs);
			if (dst->eo_u.eo_u_group.eg_objs == NULL) {
				ea_free_object(ret_obj, EUP_ALLOC);
				return (NULL);
			}
			dst->eo_u.eo_u_group.eg_nobjs = src->eo_u.eo_u_group.eg_nobjs;
		}
...
ea_get_object_tree(ea_file_t *ef, uint32_t nobj)
{
	...
	while (nobj--) {
		...
		if (obj->eo_type == EO_GROUP && obj->eo_u.eo_u_group.eg_nobjs > 0) {
			if ((obj->eo_u.eo_u_group.eg_objs = ea_get_object_tree(ef,
			    obj->eo_u.eo_u_group.eg_nobjs)) == NULL) {
				/* exacct_error set above. */
				ea_free_object(first_obj, EUP_ALLOC);
				return (NULL);
			}
