...
#define HOST_IP_A "10.10.2.1"
#define HOST_IP_B "10.10.2.2"
...
int
fmd_pceo_open(int *partner_fmd_event_p)
{
	CLIENT *cl = NULL;
	int result = -1;
	char buf[256] = {"\0"};
	...
	if (error <= 0) {
		...
		hostid = strtoul(buf, NULL, 10);
	}

	if (hostid % 2 == 1){
		cl = clnt_create(HOST_IP_B, FMD_PCEO, FMD_PCEO_VERSION_1,
			NULL);
		/* syslog(LOG_ERR, "local_hostid = %d,remote_ip = %s\n", hostid, HOST_IP_B); */
	} else if (hostid % 2 == 0) {	
		cl = clnt_create(HOST_IP_A, FMD_PCEO, FMD_PCEO_VERSION_1,
			NULL);
		/* syslog(LOG_ERR, "local_hostid = %d,remote_ip = %s\n", hostid, HOST_IP_A); */
	} else {
		syslog(LOG_ERR, "system hostid is not correct, hostid: %d\n", hostid);
		return (result);
	}

	if (!cl) {
		syslog(LOG_ERR, "create client failed\n");
		return (result);
	}

	fmd_pceo_getstate_1(*partner_fmd_event_p, &result, cl);
	/* syslog(LOG_ERR, "in = %d, out = %d\n", *partner_fmd_event_p, result); */
...
fmd_adm_stats_read(fmd_adm_t *ap, const char *name, fmd_adm_stats_t *sp)
{
	...
	if (rms.rms_err != 0) {
		xdr_free((xdrproc_t)xdr_fmd_rpc_modstat, (char *)&rms);
...
fmd_adm_module_iter(fmd_adm_t *ap, fmd_adm_module_f *func, void *arg)
{
	...
	if (rml.rml_err != 0 || rml.rml_len == 0) {
		xdr_free((xdrproc_t)xdr_fmd_rpc_modlist, (char *)&rml);
...
fmd_adm_rsrc_count(fmd_adm_t *ap, int all, uint32_t *rcp)
{
	...
	if (rrl.rrl_err != 0) {
		xdr_free((xdrproc_t)xdr_fmd_rpc_rsrclist, (char *)&rrl);
...
fmd_adm_rsrc_iter(fmd_adm_t *ap, int all, fmd_adm_rsrc_f *func, void *arg)
{
	...
	for (i = 0; i < rrl.rrl_cnt; i++) {
		...
		if (rri.rri_err != 0 && rri.rri_err != FMD_ADM_ERR_RSRCSRCH) {
			xdr_free((xdrproc_t)xdr_fmd_rpc_rsrcinfo, (char *)&rri);
			free(fmris);
			xdr_free((xdrproc_t)xdr_fmd_rpc_rsrclist, (char *)&rrl);
    ...
    fmd_adm_serd_f *func, void *arg)
{
	...
	if (rsl.rsl_err != 0 || rsl.rsl_len == 0) {
		xdr_free((xdrproc_t)xdr_fmd_rpc_serdlist, (char *)&rsl);
...
fmd_adm_xprt_iter(fmd_adm_t *ap, fmd_adm_xprt_f *func, void *arg)
{
	...
	if (rxl.rxl_err != 0) {
		xdr_free((xdrproc_t)xdr_fmd_rpc_xprtlist, (char *)&rxl);
