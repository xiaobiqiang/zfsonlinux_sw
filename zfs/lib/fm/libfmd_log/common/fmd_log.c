...
fmd_log_load_record(fmd_log_t *lp, uint_t iflags, fmd_log_record_t *rp)
{
	...
	for (obj = grp->eo_u.eo_u_group.eg_objs; obj != NULL; obj = obj->eo_next) {
		switch (obj->eo_catalog) {
		case CAT_FMA_NVLIST:
			if ((err = nvlist_unpack(obj->eo_u.eo_u_item.ei_u.ei_u_raw,
			    obj->eo_u.eo_u_item.ei_size, &rp->rec_nvl, 0)) != 0) {
		...
		case CAT_FMA_TODSEC:
			rp->rec_sec = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;

		case CAT_FMA_TODNSEC:
			rp->rec_nsec = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;

		case CAT_FMA_GROUP:
			rp->rec_nrefs += obj->eo_u.eo_u_group.eg_nobjs;
			break;
		}
	}
    ...
    fmd_log_record_t *rp, ea_object_t *grp)
{
	...
	for (obj = grp->eo_u.eo_u_group.eg_objs; obj != NULL; obj = obj->eo_next) {
		switch (obj->eo_catalog) {
		case CAT_FMA_MAJOR:
			maj = obj->eo_u.eo_u_item.ei_u.ei_u_uint32;
			break;
		case CAT_FMA_MINOR:
			min = obj->eo_u.eo_u_item.ei_u.ei_u_uint32;
			break;
		case CAT_FMA_INODE:
			ino = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;
		case CAT_FMA_OFFSET:
			off = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;
		case CAT_FMA_UUID:
			uuid = obj->eo_u.eo_u_item.ei_u.ei_u_string;
			break;
		}
	}
...
fmd_log_load_xrefs(fmd_log_t *lp, uint_t iflags, fmd_log_record_t *rp)
{
	 ...
	 */
	for (grp = rgrp->eo_u.eo_u_group.eg_objs; grp != NULL; grp = grp->eo_next) {
		if (grp->eo_catalog != CAT_FMA_GROUP)
			continue; /* ignore anything that isn't a group */

		for (obj = grp->eo_u.eo_u_group.eg_objs;
...
fmd_log_open(int abi, const char *name, int *errp)
{
	...
	for (obj = grp->eo_u.eo_u_group.eg_objs; obj != NULL; obj = obj->eo_next) {
		...
		case CAT_FMA_VERSION:
			lp->log_version = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			...
			break;
		case CAT_FMA_LABEL:
			lp->log_label = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			...
			break;
		case CAT_FMA_OSREL:
			lp->log_osrelease = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			...
			break;
		case CAT_FMA_OSVER:
			lp->log_osversion = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			...
			break;
		case CAT_FMA_PLAT:
			lp->log_platform = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			...
			break;
		case CAT_FMA_UUID:
			lp->log_uuid = strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string);
			if (lp->log_uuid == NULL) {
				ea_free_object(grp, EUP_ALLOC);
				return (fmd_log_open_err(lp, errp, EFDL_NOMEM));
			}
