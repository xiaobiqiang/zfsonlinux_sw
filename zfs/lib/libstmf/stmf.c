...
static int notifyLuActive(const char *fname);
...
static uint8_t iGetPersistMethod(void);
...
static void deleteNonActiveLus(void);
static int setDiskStandbyAccess(stmfGuid *luGuid);
static int closeDiskStandby(stmfGuid *luGuid);
...
static pthread_mutex_t persistDiskGuidLock = PTHREAD_MUTEX_INITIALIZER;
static pthread_rwlock_t openStmfRWlock = PTHREAD_RWLOCK_INITIALIZER;
...
RETRY_OPEN:
	if ((flag & OPEN_EXCL_STMF) != 0) {
		pthread_rwlock_wrlock(&openStmfRWlock);
	} else {
		pthread_rwlock_rdlock(&openStmfRWlock);
	}
	if ((*fd = open(STMF_PATH, O_NDELAY | O_RDONLY | flag)) != -1) {
		ret = STMF_STATUS_SUCCESS;
	} else {
		if (errno == EBUSY) {
			if (++retryCnt < MAX_OPEN_STMF_RETRY) {
				pthread_rwlock_unlock(&openStmfRWlock);
				usleep(100000);
				goto RETRY_OPEN;
			}
			ret = STMF_ERROR_BUSY;
 ...
 * stmfNotifyLuActive
 *
 * Purpose: notify logical unit to become active
 *
 * fname - A file name where the metadata resides
 *
 */
int
stmfNotifyLuActive(const char *fname)
{
 ...
 */
static int
notifyLuActive(const char *fname)
{
	int ret = STMF_STATUS_SUCCESS;
	int fd = 0;
	int ioctlRet;
	...
	int metaFileNameLen;
	int iluBufSize = 0;
	sbd_notify_active_t *sbdNotify = NULL;
	stmf_iocdata_t sbdIoctl = {0};
	 ...
	 * concatenation of variable length fields
	 */
	sbdNotify = (sbd_notify_active_t *)calloc(1,
	    sizeof (sbd_notify_active_t) + iluBufSize - 8);
	if (sbdNotify == NULL) {
		(void) close(fd);
		return (STMF_ERROR_NOMEM);
	}
	 ...
	 * meta data is shared with the data.
	 */
	(void) strncpy(sbdNotify->lu_meta_fname, fname, metaFileNameLen);

	sbdNotify->struct_size = sizeof (sbd_notify_active_t) + iluBufSize - 8;

	sbdIoctl.stmf_version = STMF_VERSION_1;
	sbdIoctl.stmf_ibuf_size = sbdNotify->struct_size;
	sbdIoctl.stmf_ibuf = (uint64_t)(unsigned long)sbdNotify;
 ...
 * stmfLuStandbyAccess
 *
 * Purpose: Sets lu aceess state standby
 *
 * luGuid - guid of registered logical unit
 *
 */
int
stmfLuStandbyAccess(stmfGuid *luGuid)
{
	int ret = STMF_STATUS_SUCCESS;
	stmfLogicalUnitProperties luProps;
	...
	if ((ret = stmfGetLogicalUnitProperties(luGuid, &luProps))
		...
		if (strcmp(luProps.providerName, "sbd") == 0) {
			ret = setDiskStandbyAccess(luGuid);
		} else if (luProps.status == STMF_LOGICAL_UNIT_UNREGISTERED) {
			return (STMF_ERROR_NOT_FOUND);
		} else {
			return (STMF_ERROR_INVALID_ARG);
 ...
 * stmfCloseStandbyLu
 *
 * Purpose: Close standby lu
 *
 * luGuid - guid of registered logical unit
 *
 */
int
stmfCloseStandbyLu(stmfGuid *luGuid)
{
	int ret = STMF_STATUS_SUCCESS;
	stmfLogicalUnitProperties luProps;
...
static int
setDiskStandbyAccess(stmfGuid *luGuid)
{
	int ret = STMF_STATUS_SUCCESS;
	stmf_iocdata_t sbdIoctl = {0};
	sbd_set_lu_standby_access_t sbdLu = {0};
	int ioctlRet;
	int savedErrno;
	int fd = 0;
	...
	sbdIoctl.stmf_version = STMF_VERSION_1;
	sbdIoctl.stmf_ibuf_size = sizeof (sbd_set_lu_standby_access_t);
	sbdIoctl.stmf_ibuf = (uint64_t)(unsigned long)&sbdLu;
...
static int
closeDiskStandby(stmfGuid *luGuid)
{
	int ret = STMF_STATUS_SUCCESS;
	stmf_iocdata_t sbdIoctl = {0};
	sbd_close_standby_lu_t sbdLu = {0};
	int ioctlRet;
	int savedErrno;
	int fd = 0;
	...
	sbdIoctl.stmf_version = STMF_VERSION_1;
	sbdIoctl.stmf_ibuf_size = sizeof (sbd_close_standby_lu_t);
	sbdIoctl.stmf_ibuf = (uint64_t)(unsigned long)&sbdLu;
...
static int
doUnmapLu(stmfGuid *luGuid, uint64_t offset, uint64_t length)
{
	int ret = STMF_STATUS_SUCCESS;
	int fd = 0;
	stmf_iocdata_t sbdIoctl = {0};
	...
	sbdLu = (sbd_modify_lu_t *)calloc(1, sizeof(sbd_modify_lu_t));
	if (sbdLu == NULL) {
		(void) close(fd);
		return (STMF_ERROR_NOMEM);
	}
	...
	sbdIoctl.stmf_version = STMF_VERSION_1;
	sbdIoctl.stmf_ibuf_size = sizeof(sbd_modify_lu_t);
	sbdIoctl.stmf_ibuf = (uint64_t)(unsigned long)sbdLu;
...
int
stmfUnmapLu(stmfGuid *luGuid, uint64_t offset, uint64_t length)
{
	int ret = STMF_STATUS_SUCCESS;
	stmfLogicalUnitProperties luProps;
	...
	if ((ret = stmfGetLogicalUnitProperties(luGuid, &luProps))
		...
		if (strcmp(luProps.providerName, "sbd") == 0) {
			ret = doUnmapLu(luGuid, offset, length);
		} else if (luProps.status == STMF_LOGICAL_UNIT_UNREGISTERED) {
			return (STMF_ERROR_NOT_FOUND);
		} else {
			return (STMF_ERROR_INVALID_ARG);
...
addGuidToDiskStore(stmfGuid *guid, char *filename)
{
	int ret;
	(void) pthread_mutex_lock(&persistDiskGuidLock);
	ret = persistDiskGuid(guid, filename, B_TRUE);
	(void) pthread_mutex_unlock(&persistDiskGuidLock);
	if (ret != STMF_STATUS_SUCCESS) {
		syslog(LOG_ERR, "%s: persistDiskGuid failed, filename:%s",
			__func__, filename);
	}
	return (ret);
}
...
persistDiskGuid(stmfGuid *guid, char *filename, boolean_t persist)
{
	...
	if (retryCnt != 0) {
		syslog(LOG_DEBUG, "%s: %s retry %d times, ret(%d)",
			__func__, filename, retryCnt, ret);
	}
done:
	nvlist_free(nvl);
	return (ret);
...
loadDiskPropsFromDriver(luResourceImpl *hdl, sbd_lu_props_t *sbdProps)
{
	...
	diskLu->mediaLoaded = sbdProps->slp_media_loaded;

	diskLu->activeHostId = sbdProps->slp_active_hostid;
	...
	boolean_t get_standby)
{
	...
	if (prop == STMF_LU_PROP_ACTIVE_HOST_ID) {
		(void) snprintf(propVal, *propLen, "%d",
		    diskLu->activeHostId);
		return (0);
	}
    ...
    uint64_t *setToken)
{
	...
	ret = initializeConfig();
	if (ret != STMF_STATUS_SUCCESS) {
		syslog(LOG_ERR, "%s: initializeConfig() failed, ret:0x%x",__func__, ret);
		return (ret);
	}
...
stmfClearTrace(void)
{
	 ...
	 */
	stmfIoctl.stmf_version = STMF_VERSION_1;
	stmfIoctl.stmf_ibuf_size = 0;
	stmfIoctl.stmf_ibuf = 0;
	stmfIoctl.stmf_obuf_size = 0;
	stmfIoctl.stmf_obuf = 0;
	ioctlRet = ioctl(fd, STMF_IOCTL_CLEAR_TRACE, &stmfIoctl);
...
getTraceLen(int *len)
{
	...
	stmfIoctl.stmf_ibuf_size = 0;
	stmfIoctl.stmf_ibuf = 0;
	stmfIoctl.stmf_obuf_size = 4;
	stmfIoctl.stmf_obuf = (uint64_t)(unsigned long)len;
	ioctlRet = ioctl(fd, STMF_IOCTL_GET_TRACE_POSITION, &stmfIoctl);
...
stmfGetTrace()
{
	...
	int ioctlRet;
	stmf_iocdata_t stmfIoctl;
	if ((ret = getTraceLen(&len) != STMF_STATUS_SUCCESS))
		return (ret);
	 ...
	 */
	stmfIoctl.stmf_version = STMF_VERSION_1;
	stmfIoctl.stmf_ibuf_size = 4;
	stmfIoctl.stmf_ibuf = (uint64_t)(unsigned long)&start;
	stmfIoctl.stmf_obuf_size = (uint32_t)len;
	stmfIoctl.stmf_obuf = (uint64_t)(unsigned long)buf;
	ioctlRet = ioctl(fd, STMF_IOCTL_GET_TRACE, &stmfIoctl);
