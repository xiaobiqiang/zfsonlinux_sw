...
/* macros for lost path info,  valid only in cluster system */
#define	VHCI_LOST_TARGET_PORT_MAX_NUM		18
#define	VHCI_LOST_INFO_MAX_RECORD_NUM		3

/*
 * lose paths info, valid only in cluster system
 */
typedef struct vhci_lost_path_info_s {
	int 		lun;
	int 		target_id;
	char 		target_port[VHCI_LOST_TARGET_PORT_MAX_NUM];
} vhci_lost_path_info_t;
