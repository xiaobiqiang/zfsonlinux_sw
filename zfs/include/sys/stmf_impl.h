...
#define	STMF_EVENT_ADD(h, e)		atomic_or_32(&(h), ((uint32_t)1) << (e))
#define	STMF_EVENT_REMOVE(h, e)		atomic_and_32(&(h), ~(((uint32_t)1) << (e)))
...
typedef struct stmf_i_lu {
	...
	void		*ilu_luid;
	/*  */
	struct stmf_i_lu *ilu_next_redo;
} stmf_i_lu_t;
...
/* lun transition state defines */
#define ITASK_TRANSITION_NORMAL 	0
#define ITASK_TRANSITION_WAIT 		1
#define ITASK_TRANSITION_RERUN 		2


#define ITASK_IO_STEP_NEWTASK  	0
#define ITASK_IO_STEP_XFERDONE 	1
...
typedef struct stmf_i_scsi_task {
	...
	/* transtion state data */
	kmutex_t		itask_transition_mutex;
	uint8_t			itask_transition_state;
	uint8_t			itask_io_step;
...
typedef struct stmf_worker {
	...
	hrtime_t		worker_wait_timestamp;
	hrtime_t		worker_timed_wait_timestamp;
	hrtime_t		worker_after_sig_timestamp;
	hrtime_t		worker_rec_signal_timestamp;
} stmf_worker_t;
...
typedef struct stmf_transition_worker {
	kmutex_t	lu_list_mutex;
	struct stmf_i_lu *ilu_next_redo_wait;
	struct stmf_i_lu *ilu_next_redo_wait_tail;
	struct stmf_i_lu *ilu_next_redo;
	struct stmf_i_lu *ilu_next_redo_tail;
	kthread_t	*transition_redo_thread;

	kmutex_t	worker_lock;
	kcondvar_t	worker_cv;
} stmf_transition_worker_t;
...
void stmf_dlun_init(void);
stmf_status_t stmf_dlun_fini(void);
