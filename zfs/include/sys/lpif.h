...
typedef struct stmf_lu {
	...
	void			(*lu_dbuf_xfer)(struct scsi_task *task,
...
#define	STMF_MSG_LU_DEACTIVE		3
#define	STMF_MSG_LU_SET_AVS_STATE	4
#define	STMF_MSG_LU_SET_SYNC_FLAG	5
...
void stmf_do_contrler_transition(void *ilu);
...
stmf_status_t stmf_register_lu(stmf_lu_t *lup, boolean_t proxy_reg);
...
stmf_status_t stmf_proxy_scsi_cmd(scsi_task_t *, stmf_data_buf_t *dbuf,
    uint32_t task_flags);
stmf_status_t stmf_proxy_scsi_data_res(scsi_task_t *, stmf_data_buf_t *dbuf);
...
stmf_status_t stmf_notify_avs_master_state(stmf_lu_t *lup,
	uint32_t master_state);
stmf_status_t stmf_set_remote_sync_flag(stmf_lu_t *lup,
