...
#define	SBD_IOCTL_SET_UNMPA_LU				SBD_IOCTL_DEF(11)
#define	SBD_IOCTL_NARROW_ZFS_DIRTY_MAX		SBD_IOCTL_DEF(12)
#define	SBD_IOCTL_RESTORE_ZFS_DIRTY_MAX		SBD_IOCTL_DEF(13)
...
typedef struct sbd_notify_active {
	uint32_t	struct_size;
	char		lu_meta_fname[8]; /* Can be more than 8 */
} sbd_notify_active_t;
...
typedef struct sbd_lu_props {
	...
	uint8_t		slp_media_loaded;
	uint8_t		slp_pad[3];
	uint32_t	slp_active_hostid;
	char		slp_rev[4];
	char		slp_vid[8];
	char		slp_pid[16];
	uint8_t		slp_guid[16];
