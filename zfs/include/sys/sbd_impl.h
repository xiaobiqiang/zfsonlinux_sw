...
#define SBD_SCSI_CMD_WRITE_METADATA	0x20
...
typedef struct sbd_create_standby_lu {
	...
	uint32_t  stlu_serial_no_size;
	uint32_t	stlu_rsvd;
	uint8_t	stlu_data_blocksize_shift;
	uint64_t	stlu_lu_size;
	uint8_t		stlu_guid[16];
	char		*stlu_meta_fname;
	char 	*stlu_serial_no;
	void		*stlu_active_sess;
...
int sbd_zvol_alloc_read_bufs(sbd_lu_t *sl, stmf_data_buf_t *dbuf, char *initiator_wwn);
...
void sbd_dbuf_xfer(struct scsi_task *task, uint32_t relative_offset, uint32_t len);
