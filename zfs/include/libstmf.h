...
#define	STMF_PROXY_DOOR_SVC \
	"/var/run/stmf_proxy_door_svc"
...
#define	STMF_OP_ADD	0x1
#define	STMF_OP_DELETE	0x2


typedef struct _stmf_proxy_dreq {
	char *dreq_data;
}stmf_proxy_dreq_t;

typedef struct _stmf_proxy_op_head {
	uint32_t op_type;
	uint32_t item_type;
}stmf_proxy_op_head_t;

#define	STMF_VIEW_OP	0x1
#define	STMF_HG_OP	0x2
#define	STMF_HG_MEMBER_OP	0x3
#define	STMF_TG_OP	0x4
#define	STMF_TG_MEMBER_OP	0x5
typedef struct _stmf_add_proxy_view {
	stmf_proxy_op_head_t head;
	stmfGuid lu_guid;
	stmfViewEntry view_entry;
}stmf_add_proxy_view_t;

typedef struct _stmf_remove_proxy_view {
	stmf_proxy_op_head_t head;
	stmfGuid lu_guid;
	uint32_t view_index;
}stmf_remove_proxy_view_t;

typedef struct _stmf_proxy_device {
	stmf_proxy_op_head_t head;
	stmfGroupName name;
	stmfDevid device;
}stmf_proxy_device_t;
...
int stmfNarrowZfsDirtyMax(void);
int stmfRestoreZfsDirtyMax(void);
...
int stmfNotifyLuActive(const char *fname);
int stmfInitProxyDoor(int *hdl, int fd);
int stmfLoadConfig(void);
int stmfLuStandby(stmfGuid *luGuid);
int stmfLuStandbyAccess(stmfGuid *luGuid);
int stmfCloseStandbyLu(stmfGuid *luGuid);
int stmfModifyLu(stmfGuid *luGuid, uint32_t prop, const char *propVal);
int stmfUnmapLu(stmfGuid *luGuid, uint64_t offset, uint64_t length);
