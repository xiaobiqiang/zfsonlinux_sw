...
fmdump_date(char *buf, size_t len, const fmd_log_record_t *rp)
{
	if (rp->rec_sec > LONG_MAX) {
		...
		(void) snprintf(buf, len, "0x%lx", rp->rec_sec);
	} else {
		...
		if (tod > now+60 ||
			...
			(void) snprintf(buf + sz, len - sz, ".%4.4lu",
...
usage(FILE *fp)
{
	(void) fprintf(fp, "Usage: %s [-efmvV] [-g pattern] [-c class] [-R root] [-t time] "
...
static fmd_log_filter_nvarg_t *
setupvalue(char *namevalue)
{
	fmd_log_filter_nvarg_t	*argt = NULL;
	char			*value;
	regex_t			*value_regex = NULL;
	char			errstr[128];
	int			rv;
...
main(int argc, char *argv[])
{
	...
	fmd_log_filter_t *errfv, *fltfv, *allfv, *freefv;
	uint_t errfc = 0, fltfc = 0, allfc = 0, freefc = 0;
	...
	freefv = alloca(sizeof (fmd_log_filter_t) * argc);

	while (optind < argc) {
		while ((c =
		    getopt(argc, argv, "ac:efg:Hmn:O:R:t:T:u:vV")) != EOF) {
			switch (c) {
			case 'a':
				opt_a++;
				break;
			...
			case 'n': {
				...
				freefv[freefc++] = fltfv[fltfc];
				allfv[allfc++] = fltfv[fltfc++];
				break;
			}
			case 'g': {
				fltfv[fltfc].filt_func = fmd_log_filter_string;
				fltfv[fltfc].filt_arg = setupvalue(optarg);
				freefv[freefc++] = fltfv[fltfc];
				allfv[allfc++] = fltfv[fltfc++];
				break;
			}
	...
	while(freefc--) {
		if(freefv[freefc].filt_arg != NULL) {
			if(((fmd_log_filter_nvarg_t *)(freefv[freefc].filt_arg))->nvarg_value_regex != NULL)
				free(((fmd_log_filter_nvarg_t *)(freefv[freefc].filt_arg))->nvarg_value_regex);
			free(freefv[freefc].filt_arg);
		}
	}
