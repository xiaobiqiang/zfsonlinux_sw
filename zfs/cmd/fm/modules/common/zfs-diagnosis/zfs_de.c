...
#define	FM_EREPORT_ZFS_SEGMENT_ERR		"segment_error"
...
zfs_serd_name(char *buf, uint64_t pool_guid, uint64_t vdev_guid,
    const char *type)
{
	(void) snprintf(buf, MAX_SERDLEN, "zfs_%lx_%lx_%s", pool_guid,
	    vdev_guid, type);
}
...
zfs_fm_recv(fmd_hdl_t *hdl, fmd_event_t *ep, nvlist_t *nvl, const char *class)
{
	...
	if (fmd_nvl_class_match(hdl, nvl,
	    ...
	    ZFS_MAKE_EREPORT(FM_EREPORT_ZFS_SEGMENT_ERR))) {
	    /*
		 * zfs: allocating allocated segment.
		 */
		zfs_case_solve(hdl, zcp, "fault.fs.zfs.segment_error", B_FALSE);
	} else if (fmd_nvl_class_match(hdl, nvl,
