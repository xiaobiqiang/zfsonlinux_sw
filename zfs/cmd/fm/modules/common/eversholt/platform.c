    ...
    nvpair_t *pv_nvp)
{
	...
	case DATA_TYPE_UINT64:
		...
		(void) snprintf(buf, sizeof (buf), "0x%lx", ui64);
		propv = buf;
		break;
	...
	case DATA_TYPE_BOOLEAN_VALUE:
		...
		(void) snprintf(buf, sizeof (buf), "0x%lx", (uint64_t)bool);
		propv = buf;
		break;
	...
	case DATA_TYPE_INT32:
		...
		(void) snprintf(buf, sizeof (buf), "0x%lx",
	...
	case DATA_TYPE_INT64:
		...
		(void) snprintf(buf, sizeof (buf), "0x%lx", (uint64_t)i64);
		propv = buf;
		break;
	...
	case DATA_TYPE_UINT32:
		...
		(void) snprintf(buf, sizeof (buf), "0x%lx", (uint64_t)ui32);
		propv = buf;
		break;
...
generate_envp(struct arrow *arrowp, char ***envp, int *envc, int *envplen)
{
	...
	*envp = (char **)MALLOC(sizeof (char *) * *envplen);
	...
	if (arrowp->head->myevent->enode->file == NULL) {
		...
		(void) snprintf(envvalues[3], sizeof (char *), "%d",
