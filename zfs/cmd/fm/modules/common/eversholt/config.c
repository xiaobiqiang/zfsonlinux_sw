...
{
	out(O_ALTFP|O_VERB, "%u -> %p", (uint64_t)lhs, rhs);
}
 ...
 */
struct node *
config_bycpuid_lookup(struct cfgdata *fromcfg, uint64_t id)
{
	struct config *find;
	struct node *np;
 ...
 */
static void
pconf(long flags, struct config *cp, char *buf, int offset, int limit)
{
	char *sep = "/";
	...
	if (offset)
		...
		sep = "";
	(void) snprintf(&buf[offset], limit - offset, "%s%s%ld",
	    sep, cp->s, cp->num);
	if (cp->child == NULL) {
		out(flags, "%s", buf);
