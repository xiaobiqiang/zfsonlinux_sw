...
fmd_time_sync(fmd_timeval_t *ftv, hrtime_t *hrp, uint_t samples)
{
	...
	hrtime_t hrtbase = INT64_MAX;
	hrtime_t hrtmin = INT64_MAX;
	...
	for (i = 0; i < samples; i++) {
		...
		memset(&todbase, 0, sizeof(todbase));
		if (delta < hrtmin) {
			hrtmin = delta;
			hrtbase = t0 + delta / 2;
			todbase = tod;
...
const fmd_timeops_t fmd_timeops_native = {
	(void *(*)(void))fmd_time_nop,	/* fto_init */
	(void (*)(void *))fmd_time_nop,	/* fto_fini */
	(int (*)(struct timeval*,void *))gettimeofday,			/* fto_gettimeofday */
	gethrtime,			/* fto_gethrtime */
	(void (*)(hrtime_t))fmd_time_nop,	/* fto_addhrtime */
