...
fmd_log_check_hdr(fmd_log_t *lp, const char *tag)
{
	...
	for (obj = grp->eo_u.eo_u_group.eg_objs; obj != NULL; obj = obj->eo_next) {
		...
		case CAT_FMA_VERSION:
			...
			for (fvers = 0, p = obj->eo_u.eo_u_item.ei_u.ei_u_string;
			    *p != '\0'; p++) {
				if (isdigit(*p))
					fvers = fvers * 10 + (*p - '0');
				else
			...
			if (fvers > dvers) {
				fmd_error(EFMD_LOG_INVAL, "%s: log version "
				    ...
				    lp->log_name, obj->eo_u.eo_u_item.ei_u.ei_u_string);
				ea_free_object(grp, EUP_ALLOC);
		...
		case CAT_FMA_LABEL:
			if (strcmp(obj->eo_u.eo_u_item.ei_u.ei_u_string, tag) != 0) {
				fmd_error(EFMD_LOG_INVAL, "%s: log tag '%s' "
				    "does not matched expected tag '%s'\n",
				    lp->log_name, obj->eo_u.eo_u_item.ei_u.ei_u_string, tag);
				ea_free_object(grp, EUP_ALLOC);
			...
			break;
		case CAT_FMA_UUID:
			lp->log_uuid = fmd_strdup(obj->eo_u.eo_u_item.ei_u.ei_u_string,
			    FMD_SLEEP);
	 ...
	 */
	if ((grp = ea_get_object_tree(&lp->log_ea, 1)) == NULL ||
	    grp->eo_catalog != CAT_FMA_GROUP || grp->eo_u.eo_u_group.eg_nobjs < 1 ||
	    grp->eo_u.eo_u_group.eg_objs->eo_catalog != CAT_FMA_OFFSET) {
		ea_free_object(grp, EUP_ALLOC);
		return (fmd_log_check_err(lp, EFMD_LOG_INVAL,
	...
	lp->log_skip = grp->eo_u.eo_u_group.eg_objs->eo_u.eo_u_item.ei_u.ei_u_uint64;
...
fmd_log_append(fmd_log_t *lp, fmd_event_t *e, fmd_case_t *cp)
{
	...
	ea_object_t grp0, grp1, i0, i1, i2;
	ea_object_t *items = NULL;
	ea_object_t **fe = NULL;
...
fmd_log_unpack(fmd_log_t *lp, ea_object_t *grp, off64_t off)
{
	...
	for (obj = grp->eo_u.eo_u_group.eg_objs; obj != NULL; obj = obj->eo_next) {
		switch (obj->eo_catalog) {
		case CAT_FMA_NVLIST:
			if ((err = nvlist_xunpack(obj->eo_u.eo_u_item.ei_u.ei_u_raw,
			    obj->eo_u.eo_u_item.ei_size, &nvl, &fmd.d_nva)) != 0) {
		...
		case CAT_FMA_TODSEC:
			ftv.ftv_sec = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;

		case CAT_FMA_TODNSEC:
			ftv.ftv_nsec = obj->eo_u.eo_u_item.ei_u.ei_u_uint64;
			break;
		}
	}
