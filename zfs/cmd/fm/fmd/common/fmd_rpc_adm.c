...
static int fmd_adm_get_psu_status(int * data);
static int fmd_adm_get_fan_status(int * data);

extern int ipmi_entity_psu_status(ipmi_handle_t *, ipmi_entity_t *, int *, int *);

/*
 * called if the opposite controller 
 * want to get the fan or psu status.
 * partner_fmd_event=99 indicates get psu's status,
 * else indicates get fan's status.
*/
bool_t
fmd_pceo_getstate_1_svc(int partner_fmd_event, int *result, struct svc_req *req)
{
	if (partner_fmd_event == 99)
		fmd_adm_get_psu_status(result);
	else
		fmd_adm_get_fan_status(result);

	return (TRUE);
}

static int
fmd_adm_entity_print(ipmi_handle_t *ihp, ipmi_entity_t *ep, void *data)
{
	int *p = data;
	int isrstate = 0;
	int present;

	if(ep->ie_type == IPMI_ET_PSU) {
		ipmi_entity_psu_status(ihp, ep, &present, &isrstate);
		*p |= ((present == TRUE ? 1:0) << ep->ie_instance);
		return (0);
	}
	return (0);
}

/* iterate to get psu's status */
static int
fmd_adm_get_psu_status(int * data)
{
	ipmi_handle_t *ihp;
	char *errmsg;
	int err;
	...
	*data = 0;
	if ((ihp = ipmi_open(&err, &errmsg)) == NULL) {
		syslog(LOG_ERR, "failed to open libipmi\n");
		return (1);
	}
...
/* find  LSI-CORP-SAS2X36's fan and get it's status */
static int
fmd_adm_walk_node(topo_hdl_t *thp, tnode_t *node, void *arg)
{
	int err;
	char *pval=NULL;
	uint64_t val = 0;
	...
	if ((strcmp(topo_node_name(node), "fan") != 0) || (inst > 5))
	...
	if (strcmp(pval, "LSI-CORP-SAS2X36") != 0) {
		topo_hdl_strfree(thp, pval);
		return TOPO_WALK_NEXT;
	}
	topo_hdl_strfree(thp, pval);

	if (topo_prop_get_uint64(node, "facility", "status",
			    &val, &err) != 0) {
		return TOPO_WALK_NEXT;
	}

	*data |= ((val & 0x0f) << inst*4);

	return TOPO_WALK_NEXT;
}

/* iterate to get fan's status */
static int
fmd_adm_walk_topo(topo_hdl_t *thp, int *data)
{
	int err;
	topo_walk_t *twp;
	int flag;
	...
	*data = 0;
	if ((twp = topo_walk_init(thp, "hc", fmd_adm_walk_node, data, &err))
	    == NULL) {
		syslog(LOG_ERR, "failed to walk topology\n");
		return (-1);
	}

	flag = TOPO_WALK_CHILD;
...
/* walk whole fm topo to get fan's status */
int
fmd_adm_get_fan_status(int *data)
{
	fmd_topo_t * ftp;
	topo_hdl_t *tp;
