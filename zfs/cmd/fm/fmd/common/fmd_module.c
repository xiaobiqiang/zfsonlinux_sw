...
fmd_module_unload(fmd_module_t *mp)
{
	...
	if (mp->mod_timerids != NULL) {
		fmd_idspace_apply(mp->mod_timerids,
		    (void (*)(fmd_idspace_t *, id_t,  void *))fmd_module_untimeout, mp);
...
{
	struct fm_dc_handle *dcp = NULL;
	struct fm_dc_handle **dcv;
    ...
    const fmd_modops_t *ops, const char *suffix)
{
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		/*
		 * LHL ADD
		 * for isolation snmp-trapgen plugin because we will get a sig abort
		 * when load it.
		 */
		if(!strcmp(dp->d_name, "snmp-trapgen.so"))
			continue;
#endif
		if (dp->d_name[0] == '.')
			continue; /* skip "." and ".." */
