...
static const struct file_operations fctdev_fops = {
	.open		= fct_open,
	.release	= fct_close,
	.unlocked_ioctl	= fct_ioctl,
	.owner		= THIS_MODULE,
};

static struct miscdevice fct_misc = {
	.minor		= MISC_DYNAMIC_MINOR,
	.name		= FCT_MODULE_NAME,
	.fops		= &fctdev_fops,
};

static int __init fct_init(void)
{
	int ret;

	ret = misc_register(&fct_misc);
	if (ret != 0) {
		printk(KERN_INFO "FCT: misc_register() failed %d\n", ret);
		return (ret);
	}
	/* XXX */
...
/* ARGSUSED */
static int
fct_close(struct inode *inode, struct file *file)
{
	return (0);
}
...
{
	return (copy_from_user(kernbuf, buf, size));
}
int
...
//#endif
	...
	/* issue 0002513: modified by zywang 2016-08-09 begin */
	lport->lport_abort_timeout = 300;
	/* issue 0002513: modified by zywang 2016-08-09 end */
		...
		uint16_t task_ext)
{
	...
	atomic_inc_32(&irp->irp_fcp_xchg_count);
	cmd->cmd_rp = rp;
...
fct_post_rcvd_cmd(fct_cmd_t *cmd, stmf_data_buf_t *dbuf)
{
	...
	if (cmd->cmd_type == FCT_CMD_FCP_XCHG) {
		...
		if (fds->fds_setup_dbuf != NULL) {
			...
			if (load == 1) {
				/* estimate */
				task->task_1st_xfer_len = STMF_CMD_DATA_SIZE_MAX;
			} else {
				/* zero means no hint */
...
fct_post_implicit_logo(fct_cmd_t *cmd)
{
	...
	atomic_inc_32(&irp->irp_nonfcp_xchg_count);
	atomic_inc_32(&irp->irp_sa_elses_count);
	/*
...
fct_cmd_free(fct_cmd_t *cmd)
{
	...
	if (CMD_HANDLE_VALID(cmd->cmd_handle)) {
		...
		do {
		    ...
		    old, new) != old);
		cmd->cmd_handle = 0;
		atomic_inc_32(&iport->iport_nslots_free);
		if (cmd->cmd_rp) {
			irp = (fct_i_remote_port_t *)
			    cmd->cmd_rp->rp_fct_private;
			if (cmd->cmd_type == FCT_CMD_FCP_XCHG){
				atomic_dec_32(&irp->irp_fcp_xchg_count);
			}
			else {
				atomic_dec_32(&irp->irp_nonfcp_xchg_count);
			}
		}
		rw_exit(&iport->iport_lock);
						...
						uint32_t *bufsizep)
{
	...
	switch (cmd) {
	case STMF_CMD_GET_LPORT_STATUS:
		if ((iport->iport_link_old_state == PORT_STATE_LINK_INIT_DONE &&
		   iport->iport_link_state == PORT_STATE_LINK_DOWN_CLEANING) ||
		   (iport->iport_link_state == PORT_STATE_LINK_DOWN)) {
			link_state = STMF_PORT_LINK_DOWN;
		} else {
			link_state = STMF_PORT_LINK_UP;
		}

		bcopy(&link_state, buf, sizeof(uint8_t));
		*bufsizep = sizeof(uint8_t);
	break;
...
fct_queue_scsi_task_for_termination(fct_cmd_t *cmd, fct_status_t s)
{
	...
	stmf_abort(STMF_QUEUE_TASK_ABORT, (scsi_task_t *)cmd->cmd_specific,
	    s, NULL, FCT_QUEUE_SCSI_TASK_FOR_TERMINATION);
}
	...
	for (i = 0; i < iport->iport_port->port_max_xchges; i++) {
		...
		if (do_clear &&
			...
			stmf_abort(STMF_REQUEUE_TASK_ABORT_LPORT,
			    icmd->icmd_cmd->cmd_specific, 0, NULL, FCT_RESET_FLAG_ABORT_CALLED);
		}
	}
...
MODULE_DESCRIPTION("fct implementation");
