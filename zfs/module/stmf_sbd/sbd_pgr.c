...
sbd_pgr_in_read_keys(scsi_task_t *task, stmf_data_buf_t *initial_dbuf)
{
	...
	cmn_err(CE_NOTE, "%s %d: task = %p, slu = %p, id = %d, cdb[0] = 0x%02x, cdb[1] = 0x%02x", 
		__func__, __LINE__, task, slu, task->task_ext_id, task->task_cdb[0], task->task_cdb[1]);	
...
sbd_pgr_out_register(scsi_task_t *task, stmf_data_buf_t *dbuf)
{
	...
	cmn_err(CE_NOTE, "%s %d: task = %p, slu = %p, id = %d, cdb[0] = 0x%02x, cdb[1] = 0x%02x, rsv_key = %" PRIx64 ", svc_key = %" PRIx64 ", spec_i_pt = %d, all_tg_pt = %d", 
		__func__, __LINE__, task, slu, task->task_ext_id, task->task_cdb[0], task->task_cdb[1], 
		rsv_key, svc_key, plist->spec_i_pt, plist->all_tg_pt);

	/* Handling already registered IT session */
	if (key) {

		cmn_err(CE_NOTE, "%s %d: key isn't null", __func__, __LINE__);
...
sbd_pgr_reg_done:
	...
	cmn_err(CE_NOTE, "%s %d pgr->pgr_flags = %d, plist->aptpl = %d", __func__, __LINE__, pgr->pgr_flags, plist->aptpl);

	if (pgr->pgr_flags & SBD_PGR_APTPL || plist->aptpl) {
		if (plist->aptpl) {
			cmn_err(CE_NOTE, "%s %d set SBD_PGR_APTPL flag", __func__, __LINE__);
			PGR_SET_FLAG(pgr->pgr_flags, SBD_PGR_APTPL);
		}
		else {
			cmn_err(CE_NOTE, "%s %d clear SBD_PGR_APTPL flag, shouldn't clear", __func__, __LINE__);
...
sbd_pgr_out_preempt(scsi_task_t *task, stmf_data_buf_t *dbuf)
{
	...
	if (pr_out->action == PR_OUT_PREEMPT_ABORT) {
		stmf_abort(STMF_QUEUE_ABORT_LU, task, STMF_ABORTED,
		    (void *)slu->sl_lu, SBD_PGR_OUT_PREEMPT);
	}
	stmf_scsilib_send_status(task, STATUS_GOOD, 0);
}
