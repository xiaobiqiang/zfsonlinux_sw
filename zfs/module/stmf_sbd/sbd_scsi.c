					...
					struct stmf_data_buf *dbuf)
{
	...
	if ( (task->task_additional_flags&TASK_PPPT_LU_DBUF) ){ 
		...
		if (ret != 0) {
			cmn_err(CE_WARN,   "%s  task = %p read data failed",__func__, (void *)task);
...
int sbd_enable_standby_write = 1;
volatile int sbd_use_hp_host = 1;

static void get_scsi_inititator_wwn(char *wwn)
{
	...
	while (scmd->len && scmd->nbufs < task->task_max_nbufs) {
		...
		*/
		...
		if (ret != 0) {
			...
			kmem_free(inititator_wwn, wwwn_len + 1);
			return;
		}
				...
				struct stmf_data_buf *dbuf)
{
	if (dbuf->db_xfer_status != STMF_SUCCESS) {
		cmn_err(CE_WARN,   "%s  task = %p dbuf->db_xfer_status=%d, abort self",__func__, (void *)task,(int)dbuf->db_xfer_status);
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_READ_XFER_COMPLETION);
		return;
	}
	...
	if (scmd->len == 0 || scmd->flags & SBD_SCSI_CMD_XFER_FAIL) {
		...
		scmd->nbufs--;
		if (scmd->nbufs){
			cmn_err(CE_WARN,   "%s  task = %p scmd->nbufs=%x is not zero ************ ",__func__, (void *)task,scmd->nbufs);
	...
	if (dbuf->db_flags & DB_DONT_REUSE) {
		...
		maxsize = (scmd->len > (STMF_CMD_DATA_SIZE_MAX)) ? STMF_CMD_DATA_SIZE_MAX : scmd->len;
		minsize = maxsize >> 2;
		do {
			old_minsize = minsize;
		...
		if (dbuf == NULL) {
			...
			if (scmd->nbufs == 0) {
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_ALLOC_FAILURE, NULL, SBD_HANDLE_READ_XFER_COMPLETION);
			}
			return;
		}
	}
				...
				struct stmf_data_buf *dbuf)
{
	...
	if (scmd->flags & SBD_SCSI_CMD_ACTIVE) {
		...
		scmd->flags &= ~SBD_SCSI_CMD_ACTIVE;
		stmf_abort(STMF_QUEUE_TASK_ABORT, task, xfer_status, NULL, SBD_HANDLE_SGL_READ_XFER_COMPLETION);
	}
}
				...
				struct stmf_data_buf *dbuf)
{
	...
	*/
	...
	if (scmd->flags & SBD_SCSI_CMD_ACTIVE) {
		...
		scmd->flags &= ~SBD_SCSI_CMD_ACTIVE;
		ASSERT(xfer_status != STMF_SUCCESS);
		stmf_abort(STMF_QUEUE_TASK_ABORT, task, xfer_status, NULL, SBD_HANDLE_SGL_WRITE_XFER_COMPLETION);
	}
}
...
void sbd_read_length (char * lun_name,  char *port_name, uint64_t lun_offset, uint64_t len)
{
}
...
sbd_handle_read(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	...
	sbd_read_length(sl->sl_alias, task->task_lport->lport_alias, laddr, len);
	...
	if (len != task->task_expected_xfer_length ) {
		...
		len = (len > task->task_expected_xfer_length) ?
		    task->task_expected_xfer_length : len;
	} else if(task->task_additional_flags&TASK_PPPT_LU_DBUF) {
		fast_path = 0;
	} else {
		fast_path = 1;
	}
	...
	if (sbd_zcopy & (2|1) &&		/* Debug switch */
		...
		(task->task_additional_flags & TASK_AF_ACCEPT_LU_DBUF) && /* PP allows it */
	...
	if (initial_dbuf == NULL) {
		...
		maxsize = (len > (STMF_CMD_DATA_SIZE_MAX)) ? STMF_CMD_DATA_SIZE_MAX : len;
		minsize = maxsize >> 2;
		do {
			old_minsize = minsize;
    ...
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	...
	if (dbuf == NULL) {
		...
		maxsize = (scmd->len > (STMF_CMD_DATA_SIZE_MAX)) ? STMF_CMD_DATA_SIZE_MAX :
		    scmd->len;
		minsize = maxsize >> 2;
		do {
			old_minsize = minsize;
		...
		if (dbuf == NULL) {
			if (scmd->nbufs == 0) {
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_ALLOC_FAILURE, NULL, SBD_DO_WRITE_XFER);
			}
			return;
		}
	}
	...
	if (scmd->flags & SBD_SCSI_CMD_WRITE_METADATA)
		dbuf->db_flags |= DB_WRITE_META_DATA;
	(void) stmf_xfer_data(task, dbuf, 0);
	scmd->nbufs++; /* outstanding port xfers and bufs used */
	scmd->len -= len;
	scmd->current_ro += len;

	if ((scmd->len != 0) && (scmd->nbufs < bufs_to_take) && (task->task_max_nbufs >1)) {
...
/* 
 * Don't rush, let's task things slow and step by step,
 * Don't fire parallel xfer.
 * In standby write, the work we do local is copy the dbuf to proxy dbuf,
 * it won't take too much time, so there's no hurt wait that to complete then
 * start the next transfer.
 */
void
sbd_do_standby_write_xfer(struct scsi_task *task, sbd_cmd_t *scmd,
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	uint32_t len;
	...
	if (dbuf == NULL) {
		...
		if (dbuf == NULL) {
			if (scmd->nbufs == 0) {
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_ALLOC_FAILURE, NULL, SBD_DO_STANDBY_WRITE_XFER);
			}
			return;
		}
...
void
sbd_handle_active_write_xfer_completion(struct scsi_task *task, sbd_cmd_t *scmd,
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	sbd_lu_t *sl = (sbd_lu_t *)task->task_lu->lu_provider_private;
	uint64_t laddr;
	...
	if (dbuf->db_xfer_status != STMF_SUCCESS) {
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_ACTIVE_WRITE_XFER_COMPLETION);
		return;
	}

	if (scmd->flags & SBD_SCSI_CMD_XFER_FAIL) {
		cmn_err(CE_WARN,   "%s  task = %p  scmd->flags=%x err dbuf->db_flags=%x",__func__, (void *)task,scmd->flags,dbuf->db_flags);
		goto WRITE_XFER_DONE;
	}

	if ((scmd->len != 0) && !(dbuf->db_flags & DB_TYPE_PPPT) && task->task_max_nbufs > 1) {
		/*
		 * If the transfer is not taken place in pppt, initiate the
		 * next port xfer to occur in parallel with writing this buf.
		 */
		sbd_do_write_xfer(task, scmd, NULL, 0);
	}
	...
	//cmn_err(CE_WARN,   "%s	task = %p to task->task_additional_flags = %x",__func__, task,task->task_additional_flags);
...
WRITE_XFER_DONE:
	if (scmd->len == 0 || scmd->flags & SBD_SCSI_CMD_XFER_FAIL) {
		...
		if (scmd->flags & SBD_SCSI_CMD_XFER_FAIL) {
			...
			if ((scmd->flags & SBD_SCSI_CMD_SYNC_WRITE) &&
			    (sbd_flush_data_cache(sl, 0) != SBD_SUCCESS)) {
				cmn_err(CE_WARN,"%s(): task:0x%p write finish: STMF_SAA_WRITE_ERROR", __func__, task);
				stmf_scsilib_send_status(task, STATUS_CHECK,
							 STMF_SAA_WRITE_ERROR);
			} else {
				stmf_scsilib_send_status(task, STATUS_GOOD, 0);
...
static struct stmf_data_buf *
sbd_alloc_proxy_dbuf(uint32_t size)
{
	struct stmf_data_buf *dbuf;
	uint8_t *buf;
	...
	dbuf = kmem_zalloc(sizeof(*dbuf), KM_SLEEP);
	if (!dbuf) {
		kmem_free(buf, size);
		return NULL;
	}
...
void
sbd_handle_standby_write_xfer_completion(struct scsi_task *task, sbd_cmd_t *scmd,
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	//struct stmf_data_buf *task_dbuf = (struct stmf_data_buf *)task->task_dbuf;
	//uint8_t *dst;
	//uint32_t buflen, iolen;
	//int ndx;
	//stmf_status_t ret;
	...
	if (dbuf->db_xfer_status != STMF_SUCCESS) {
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_STANDBY_WRITE_XFER_COMPLETION);
		return;
	}

	/* Copy the data into the proxy dbuf */
	/*
	dst = task_dbuf->db_sglist[0].seg_addr + dbuf->db_relative_offset -
		task_dbuf->db_relative_offset;
	...
	for (buflen = 0, ndx = 0; (buflen < dbuf->db_data_size) &&
		...
		if (iolen == 0)
			break;
		bcopy(dbuf->db_sglist[ndx].seg_addr, dst, iolen);
		buflen += iolen;
		dst += iolen;
	}
	*/
	...
	/* Kick out the rest of data transfer. */
	//this is not allowed in standby
	/*
	if (scmd->len != 0) {
		sbd_do_standby_write_xfer(task, scmd, dbuf, 0);
		return;
	}
	*/
	...
	if (stmf_proxy_scsi_data_res(task, dbuf)) {
		//stmf_scsilib_send_status(task, STATUS_CHECK,
    ...
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	...
	if (!(itask->itask_flags & ITASK_PROXY_TASK)){ 
		sbd_handle_active_write_xfer_completion(task, scmd, dbuf,
							dbuf_reusable);
	} else { 
		sbd_handle_standby_write_xfer_completion(task, scmd, dbuf,
							 dbuf_reusable);
	}
	#else
	...
	if (sl->sl_access_state == SBD_LU_ACTIVE ){
		sbd_handle_active_write_xfer_completion(task, scmd, dbuf,
							dbuf_reusable);
	} else if(sl->sl_access_state == SBD_LU_STANDBY || sl->sl_access_state == SBD_LU_TRANSITION_TO_ACTIVE){
		sbd_handle_standby_write_xfer_completion(task, scmd, dbuf,
							 dbuf_reusable);
	} else if(sl->sl_access_state == SBD_LU_TRANSITION_TO_STANDBY){
		stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_LU_NO_ACCESS_UNAVAIL);
	}
	#endif
...
void sbd_write_length(char * lu_name,  char *port_name, uint64_t addr, uint64_t length, uint32_t cmd)
{
	
}

void
sbd_handle_active_write(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	uint64_t lba, laddr;
	uint32_t len;
	...
	if (sync_wr_flag)
		is_meta_data = task->task_cdb[1] & 0x08;
	else
		is_meta_data = task->task_cdb[1] & 0x04;

	laddr = lba << sl->sl_data_blocksize_shift;
	len <<= sl->sl_data_blocksize_shift;

	sbd_write_length(sl->sl_alias,  task->task_lport->lport_alias,  laddr,  len, op);
	...
	if ((initial_dbuf != NULL) && (task->task_flags & TF_INITIAL_BURST)) {
		if (initial_dbuf->db_data_size > len) {
			if (initial_dbuf->db_data_size >
				...
				/* protocol error */
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_INVALID_ARG, NULL, SBD_HANDLE_ACTIVE_WRITE);
				return;
			}
	...
	if (task->task_lu_private) {
		...
		scmd = (sbd_cmd_t *)kmem_zalloc(sizeof (sbd_cmd_t), KM_SLEEP);
		task->task_lu_private = scmd;
	...
	if (is_meta_data)
		scmd->flags |= SBD_SCSI_CMD_WRITE_METADATA;
	...
	if(task->task_additional_flags &TASK_PPPT_LU_DBUF){
		//cmn_err(CE_WARN,   "%s  task = %p to run sbd_do_write_xfer",__func__, (void *)task);
...
void
sbd_handle_standby_write(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	uint64_t lba, laddr;
	uint32_t len;
	uint8_t op = task->task_cdb[0], do_immediate_data = 0;
	sbd_lu_t *sl = (sbd_lu_t *)task->task_lu->lu_provider_private;
	stmf_data_buf_t *dbuf;
	uint8_t	sync_wr_flag = 0, is_meta_data = 0;
	stmf_status_t ret;
	...
	if ((initial_dbuf != NULL) && (task->task_flags & TF_INITIAL_BURST)) {
		if (initial_dbuf->db_data_size > len) {
			if (initial_dbuf->db_data_size >
				...
				/* protocol error */
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_INVALID_ARG, NULL, SBD_HANDLE_STANDBY_WRITE);
				return;
			}
			initial_dbuf->db_data_size = len;
		}
	...
	/* Send the command to peer then wait for data req */
	if (do_immediate_data) {
		/*
		 * Account for data passed in this write command
		 */
		...
		dbuf->db_xfer_status = STMF_SUCCESS;
		task->task_nbytes_transferred += dbuf->db_data_size;
		ret = stmf_proxy_scsi_cmd(task, dbuf, ITASK_PROXY_TASK);

	} else {
		ret = stmf_proxy_scsi_cmd(task, NULL, ITASK_PROXY_TASK);
	}
...
sbd_handle_write(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	...
	if ((sl->sl_access_state == SBD_LU_ACTIVE))  {
		...
		sbd_handle_standby_write(task, initial_dbuf);
	} else if(sl->sl_access_state == SBD_LU_TRANSITION_TO_STANDBY){
		cmn_err(CE_WARN,   "%s  task = %p lun sl_access_state stat is wrong",__func__, (void *)task);
		stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_LU_NO_ACCESS_UNAVAIL);
	}
	return;
				...
				struct stmf_data_buf *dbuf)
{
	...
	if (dbuf->db_xfer_status != STMF_SUCCESS) {
		...
		cmn_err(CE_WARN,   "%s	error task = %p cdb=%x flags=%x",__func__, (void *)task,task->task_cdb[0],itask->itask_flags);
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_SHORT_READ_XFER_COMPLETION);
		return;
	}
	task->task_nbytes_transferred = dbuf->db_data_size;
	scmd->flags &= ~SBD_SCSI_CMD_ACTIVE;
    ...
    stmf_data_buf_t *dbuf, uint32_t cdb_xfer_size)
{
	...
	if (dbuf == NULL) {
		...
		if (dbuf == NULL) {
			...
			stmf_abort(STMF_QUEUE_TASK_ABORT, task,
			    STMF_ALLOC_FAILURE, NULL, SBD_HANDLE_SHORT_WRITE_TRANSFERS);
			return;
		}
		...
		if (dbuf->db_data_size < cdb_xfer_size) {
			stmf_abort(STMF_QUEUE_TASK_ABORT, task,
			    STMF_ABORTED, NULL, SBD_HANDLE_SHORT_WRITE_TRANSFERS);
			return;
		}
		dbuf->db_data_size = cdb_xfer_size;
    ...
    stmf_data_buf_t *dbuf)
{
	...
	if ((dbuf->db_xfer_status != STMF_SUCCESS) ||
	    ...
	    cmn_err(CE_WARN,   "%s  do_stmf_abort task = %p",__func__, (void *)task);
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_SHORT_WRITE_XFER_COMPLETION);
		return;
	}
	...
	default:
		...
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    STMF_ABORTED, NULL, SBD_HANDLE_SHORT_WRITE_XFER_COMPLETION);
	}
}
...
sbd_write_same_data(struct scsi_task *task, sbd_cmd_t *scmd)
{
	 ...
	 */
	big_buf = kmem_alloc(big_buf_size, KM_NOSLEEP);
    ...
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	...
	if (dbuf->db_xfer_status != STMF_SUCCESS) {
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
		    dbuf->db_xfer_status, NULL, SBD_HANDLE_WRITE_SAME_XFER_COMPLETION);
		return;
	}
    ...
    struct stmf_data_buf *dbuf, uint8_t dbuf_reusable)
{
	...
	if (dbuf == NULL) {
		...
		if (dbuf == NULL) {
			if (scmd->nbufs == 0) {
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_ALLOC_FAILURE, NULL, SBD_DO_WRITE_SAME_XFER);
			}
			return;
		}
	}
	...
	if ((initial_dbuf != NULL) && (task->task_flags & TF_INITIAL_BURST)) {
		if (initial_dbuf->db_data_size > len) {
			if (initial_dbuf->db_data_size >
				...
				/* protocol error */
				stmf_abort(STMF_QUEUE_TASK_ABORT, task,
				    STMF_INVALID_ARG, NULL, SBD_HANDLE_WRITE_SAME);
				return;
			}
...
/*
void printBufinLog(char *prompt, uint8_t id,uint8_t *buf, uint32_t len)
{
	char *printbuf;
	uint32_t i;
	...
	cmn_err(CE_WARN,   "%s:%02x < %s >  ",prompt,id,printbuf);
...
sbd_handle_inquiry(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	...
	if ((((cdbp[1] & 1) == 0) && cdbp[2]) || cdbp[5]) {
		cmn_err(CE_WARN,   "%s  task = %p cdb2=%02x cdb5=%02x",__func__, (void *)task,cdbp[2],cdbp[5]);
		stmf_scsilib_send_status(task, STATUS_CHECK,
		    STMF_SAA_INVALID_FIELD_IN_CDB);
	...
	if ((cdbp[1] & 1) == 0) {
		...
		//printBufinLog("stand_inquiry",0,p,min(cmd_size, xfer_size));

		sbd_handle_short_read_transfers(task, initial_dbuf, p, cmd_size,
		    min((uint32_t)cmd_size, (uint32_t)xfer_size));
	...
	case 0x85:
		if (mgmt_url_size == 0) {
			cmn_err(CE_WARN,   "%s  task = %p mgmt_url_size 0",__func__, (void *)task);
			stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_INVALID_FIELD_IN_CDB);
				goto err_done;
		}
	...
	case 0xb0:
		if ((sl->sl_flags & SL_UNMAP_ENABLED) == 0) {
			cmn_err(CE_WARN,   "%s b0 task = %p SL_UNMAP_ENABLED",__func__, (void *)task);
			stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_INVALID_FIELD_IN_CDB);
			goto err_done;
		}
		...
		/* optimal transfer length granularity, 64KB */
		p[7] = 0x80;

		/* optimal transfer length */
		s = sl->sl_blksize >> sl->sl_data_blocksize_shift;
		p[15] = s & ((uint64_t)0x0000000000000000ull);
		p[14] = (s & ((uint64_t)0x000000000000FF00ull)) >> 8;
		p[13] = (s & ((uint64_t)0x0000000000FF0000ull)) >> 16;
		p[12] = (s & ((uint64_t)0x00000000FF000000ull)) >> 24;
	...
	case 0xb2:
		if ((sl->sl_flags & SL_UNMAP_ENABLED) == 0) {
			cmn_err(CE_WARN,   "%s b2 task = %p SL_UNMAP_ENABLED",__func__, (void *)task);
			stmf_scsilib_send_status(task, STATUS_CHECK,
			    STMF_SAA_INVALID_FIELD_IN_CDB);
			goto err_done;
		}
	...
	default:
		cmn_err(CE_WARN,   "%s default task = %p SL_UNMAP_ENABLED",__func__, (void *)task);
		stmf_scsilib_send_status(task, STATUS_CHECK,
		    STMF_SAA_INVALID_FIELD_IN_CDB);
		goto err_done;
	}

	//printBufinLog("pagedata",cdbp[2],p,min(cmd_size, xfer_size));
	sbd_handle_short_read_transfers(task, initial_dbuf, p, cmd_size,
	    min((uint32_t)cmd_size, (uint32_t)xfer_size));
err_done:
	kmem_free(p, bsize);
...
sbd_new_task(struct scsi_task *task, struct stmf_data_buf *initial_dbuf)
{
	...
	if (sl->sl_access_state == SBD_LU_STANDBY || sl->sl_access_state == SBD_LU_TRANSITION_TO_ACTIVE) {
		...
		if (cdb0 != SCMD_INQUIRY &&
		    ...
		    !(cdb0 == SCMD_MAINTENANCE_IN &&
		    (cdb1 & 0x1F) == 0x0A)) {
		    cmn_err(CE_WARN, "%s error not support scsi cmd cdb=%x.%x ",__func__, cdb0,cdb1);
			stmf_scsilib_send_status(task, STATUS_CHECK,
			...
			default:
				...
				if (st_ret != STMF_SUCCESS) {
					...
					if ((cdb0 == SCMD_READ) || (cdb0 == SCMD_WRITE)) {
						cmn_err(CE_NOTE, "%s cdb=%x.%x is reserverd %p",__func__, task->task_cdb[0],cdb1,task);
						return;
					}
					else{
						if(sbd_use_hp_host){
							...
							itask->itask_flags &= ~ITASK_PROXY_TASK;
						
							cmn_err(CE_NOTE, "%s cdb=%02x%02x is forced to exec standby  %p flags=%x",__func__, task->task_cdb[0],cdb1,task,itask->itask_flags);
							goto exec_notrw_io;
...
exec_notrw_io:
	...
	if ((cdb0 == SCMD_MAINTENANCE_IN) &&
	    ((cdb1 & 0x1F) == SSVC_ACTION_GET_TARGET_PORT_GROUPS)) {
...
void
sbd_dbuf_xfer(struct scsi_task *task, uint32_t relative_offset, uint32_t len)
{
	...
	/* Free old task_dbuf and alloc new one */
	/*
	if (task->task_dbuf) {
		sbd_free_proxy_dbuf((struct stmf_data_buf *)task->task_dbuf);
		task->task_dbuf = NULL;
	}

	task_dbuf = sbd_alloc_proxy_dbuf(len);
	if (!task_dbuf) {
		stmf_abort(STMF_QUEUE_TASK_ABORT, task,
			   STMF_ALLOC_FAILURE, NULL);
		return;
	}
	task->task_dbuf = task_dbuf;
	task_dbuf->db_relative_offset = relative_offset;
	*/
	scmd->current_ro = relative_offset;
	scmd->len = len;
	scmd->flags = SBD_SCSI_CMD_ACTIVE;
	...
	switch (op) {
		case SCMD_WRITE:
			scmd->cmd_type = SBD_CMD_SCSI_WRITE;
			sbd_do_standby_write_xfer(task, scmd, NULL, 0);
			break;
		default:
			break;
	}
...
sbd_task_free(struct scsi_task *task)
{
	if (task->task_lu_private) {
		sbd_cmd_t *scmd = (sbd_cmd_t *)task->task_lu_private;
		if (scmd->flags & SBD_SCSI_CMD_ACTIVE && scmd->len!= 0) {
			cmn_err(CE_PANIC, "cmd is active, and the len:0x%x, task = %p",
			    scmd->len, (void *)task);
		}
...
sbd_ctl(struct stmf_lu *lu, int cmd, void *arg)
{
	...
	case STMF_ACK_LU_OFFLINE_COMPLETE:
		...
		break;
	case STMF_CMD_GET_LU_STATUS:
		*((uint8_t*)arg) = sl->sl_access_state;
	...
	case STMF_CMD_SET_LU_STATUS:
		while (1) {
			...
			if (sl->sl_trans_op == SL_OP_NONE) {
				sl->sl_trans_op = SL_OP_MODIFY_LU;
				sl->sl_access_state = *((uint8_t*)arg);
				mutex_exit(&sl->sl_lock);
				break;
			}
			mutex_exit(&sl->sl_lock);
