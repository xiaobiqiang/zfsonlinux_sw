...
static int stmf_lun_compare(void *arg1, void *arg2)
{
	stmf_lun_map_item_t *item1;
	sbd_lu_t *sl1;
	u_longlong_t sl1_no;
	stmf_lun_map_item_t *item2;
	sbd_lu_t *sl2;
	u_longlong_t sl2_no;

	item1 = (stmf_lun_map_item_t *)arg1;
	item2 = (stmf_lun_map_item_t *)arg2;

	sl1 = (sbd_lu_t *)item1->map_ent->ent_lu ->lu_provider_private;
	sl2 = (sbd_lu_t *)item2->map_ent->ent_lu ->lu_provider_private;
	if (!sl1->sl_serial_no_size||!sl2->sl_serial_no_size)
		return (-1);

	if (!sl1->sl_serial_no || !sl2->sl_serial_no)
		return (-1);
	
	(void )ddi_strtoull(sl1->sl_serial_no, NULL, 0, &sl1_no);
	(void )ddi_strtoull(sl2->sl_serial_no, NULL, 0, &sl2_no);
	
	if (sl1_no > sl2_no) 
		return (1);
	else if (sl1_no < sl2_no) 
		return (-1);
...
{
	sbd_lu_t *sl;
	stmf_lun_map_ent_t *item;
	stmf_xfer_data_t *xd;
	stmf_lun_map_item_t *lun_map_array;
	uint16_t nluns, ent;
	uint32_t alloc_size, data_size;
	int i;

	nluns = sm->lm_nluns;
	cmn_err(CE_NOTE, "%s nluns = %d", __func__, nluns);
	
	lun_map_array = kmem_zalloc(nluns * sizeof(stmf_lun_map_item_t), KM_SLEEP);
	...
	if (xd == NULL) {
		cmn_err(CE_NOTE, "%s xd is NULL", __func__);
		return (NULL);
	}
	...
	*((uint32_t *)xd->buf) = BE_32(data_size - 8);
	if (nluns == 0) {
		cmn_err(CE_NOTE, "%s nluns = 0 return", __func__);
		return (xd);
	}
	...
	for (i = 0; ((i < sm->lm_nentries) && (ent < nluns)); i++) {
		if (sm->lm_plus[i] == NULL)
			continue;
		lun_map_array[ent].map_ent = (stmf_lun_map_ent_t *)sm->lm_plus[i];
		lun_map_array[ent].map_no = i;
		item = (stmf_lun_map_ent_t *)sm->lm_plus[i];
		sl = (sbd_lu_t *)item->ent_lu ->lu_provider_private;
		
		cmn_err(CE_WARN, "lun[%d] = serial_no =[%s]",i,sl->sl_serial_no);
		ent ++;
	}
	sort((void *)lun_map_array, ent, sizeof(stmf_lun_map_item_t), 
	    (int (*)(const void *, const void *))stmf_lun_compare, NULL);
	
	for (i = 0;  (i  < nluns); i++) {
		xd->buf[8 + (i << 3) + 1] = (uchar_t)lun_map_array[i].map_no;
		xd->buf[8 + (i  << 3) + 0] = ((uchar_t)(lun_map_array[i].map_no >> 8));
		cmn_err(CE_WARN, "lun[%d] = %x,%x ",i, (uchar_t)lun_map_array[i].map_no,(uchar_t)(lun_map_array[i].map_no >> 8));
	}

	ASSERT(ent == nluns);
	kmem_free(lun_map_array, sizeof(stmf_lun_map_item_t) *  nluns);
	return (xd);
}
		...
		uint8_t *lu_nbr)
{
	...
	bzero(initiator_id,sizeof(initiator_id));
	bzero(target_id,sizeof(target_id));
	ASSERT(mutex_owned(&stmf_state.stmf_lock));
	...
	bcopy(ilport->ilport_lport->lport_id->ident, target_id,
		ilport->ilport_lport->lport_id->ident_length);
	
	cmn_err(CE_WARN, "%s luNbr=%d add to session ilu_alias=%s guid=%s initiator_id=%s target_id=%s ss_session_id=0x%"PRIx64"",
		__func__, luNbr, ilu->ilu_alias, ilu->ilu_ascii_hex_guid, initiator_id, target_id, iss->iss_ss->ss_session_id);
...
int stmf_get_ent_no_from_map(stmf_lun_map_t *sm, uint8_t *identp, uint16_t size, int *index)
{
	int entry = 0;
	for (; entry < sm->lm_nentries; entry ++) {
		stmf_lun_map_ent_t *lun_map_ent = (stmf_lun_map_ent_t *)sm->lm_plus[entry];
		if (lun_map_ent == NULL) {
			/* map info error, to finish */
			goto FINISH;
		}
		if (bcmp(identp, lun_map_ent->ent_lu->lu_id->ident, size) == 0) {
			* index = entry;
			return (0);
		}
	}
