...
/*
 * log2 fraction of the zio_arena to keep free.
 */
 ...
 * to half of all memory in user space which is irimarily used for testing.
 */
static uint64_t
arc_all_memory(void)
{
#ifdef _KERNEL
	return (MIN(ptob(physmem), vmem_size(heap_arena, VMEM_FREE | VMEM_ALLOC)));
#else
...
//#if defined(__i386)
	available_memory = MIN(available_memory, vmem_size(heap_arena, VMEM_FREE));
//#endif
	...
	if (zio_arena != NULL) {
		n = (int64_t)vmem_size(zio_arena, VMEM_FREE) -
			(vmem_size(zio_arena, VMEM_ALLOC) >> arc_zio_arena_free_shift);
		if (n < lowest) {
			lowest = n;
			r = FMR_ZIO_ARENA;
		}
