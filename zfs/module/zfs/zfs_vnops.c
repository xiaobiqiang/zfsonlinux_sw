...
top:
	...
	if(zsb->z_os->os_is_group && zsb->z_os->os_is_master && bUpdateMaster2 &&
		(flags & FBackupMaster) == 0 && error == 0 && NULL == strstr(name, SMB_STREAM_PREFIX)){
		dmu_objset_name(zsb->z_os, os_name);
		oblique_line = strstr(os_name, "/");
		if (oblique_line) {
			*oblique_line = '_';
...
top:
	...
	if(zsb->z_os->os_is_group && zsb->z_os->os_is_master && (flags & FBackupMaster) == 0 && error == 0){
...
zfs_readdir(struct inode *ip, struct dir_context *ctx, cred_t *cr, int flags)
{
	...
	if (optype == VN_OP_CLIENT){
		maxentrynum = ((struct ctx_struct *)ctx->dirent)->count;
		ZFS_ENTER(zsb);
		error = zfs_client_readdir(ip, ctx, maxentrynum, cr, flags);
		ZFS_EXIT(zsb);
		return (error);
	}		
...
zfs_readdir_server(struct inode *ip, uio_t *uio, cred_t *cr, int *eofp, int flags)
{
	...
	int		outcount;
	size_t		bufsize;
	int maxentrynum = (int)uio->uio_fmode;
...
top:
	...
	if(zsb->z_os->os_is_group && zsb->z_os->os_is_master && (flags & FBackupMaster) == 0 && err == 0){
...
top:
	...
	if(zsb->z_os->os_is_group && zsb->z_os->os_is_master && bUpdateMaster2
		...
		&& NULL == strstr(tnm, SMB_STREAM_PREFIX)){
    ...
    struct inode **ipp, cred_t *cr, int flags)
{
	...
	if (optype == VN_OP_CLIENT){
		ZFS_ENTER(zsb);
		error = zfs_client_symlink(dip, name, vap, link, cr, flags);
		ZFS_EXIT(zsb);
		return (error);
	}
	bzero(&group_id, sizeof(zfs_group_object_t));
