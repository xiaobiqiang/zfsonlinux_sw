...
void zfs_group_znode_copy_phys(znode_t *zp, zfs_group_phys_t *dst_phys, boolean_t nosa)
{
	...
	dst_phys->zp_gid = zp->z_gid;
	dst_phys->zp_uid = zp->z_uid;
	dst_phys->zp_gen = zp->z_gen;
	dst_phys->zp_size = zp->z_size ;
	dst_phys->zp_links = zp->z_links;
	...
	if (!nosa && zp->z_sa_hdl != NULL) {
		if ((zp->z_id == zp->z_zsb->z_root) ||
		    ...
		    && (zp->z_group_id.data_objset == dmu_objset_id(zp->z_zsb->z_os)))) {
		    sa_object_size(zp->z_sa_hdl,
			    (uint32_t *)&dst_phys->zp_blksz,
			    (u_longlong_t *)&dst_phys->zp_nblocks);
		} else {
			if (zp->z_group_id.data_spa == spa_guid(dmu_objset_spa(zp->z_zsb->z_os))) {
				sa_object_size(zp->z_sa_hdl, (uint32_t *)&blksz, (u_longlong_t *)&nblks);
				dst_phys->zp_blksz = blksz;
				dst_phys->zp_nblocks = nblks;
			} else {
				sa_lookup(zp->z_sa_hdl, SA_ZPL_NBLKS(zp->z_zsb),
				    &dst_phys->zp_nblocks,
				    sizeof (uint64_t));
				sa_lookup(zp->z_sa_hdl, SA_ZPL_BLKSZ(zp->z_zsb),
				    &dst_phys->zp_blksz,
				    sizeof (uint64_t));
			}
		}
...
//	int r;
	...
	zfs_group_build_header(zp->z_zsb->z_os, msg_header, ZFS_GROUP_CMD_NAME,
	    SHARE_WAIT, op,
...
//			new_znode = *zp;
	...
	zfs_group_build_header_backup(zp->z_zsb->z_os, msg_header, ZFS_GROUP_CMD_NAME_BACKUP,
	    SHARE_WAIT, op,
	    request_length, reply_length, dst_spa, dst_os, dst_object,
	    dst_object,
	    z_group_id->data_spa,
    ...
    void *ptr, uint64_t io_flags, data_direction_t direction)
{
	...
	if (zp->z_id == zp->z_zsb->z_root) {
		zp->z_group_id.master_spa = zp->z_zsb->z_os->os_master_spa;
		zp->z_group_id.master_objset = zp->z_zsb->z_os->os_master_os;
		zp->z_group_id.master_object = zp->z_zsb->z_os->os_master_root;
	}
    ...
    msg_orig_type_t msg_orig, zfs_multiclus_node_type_t m_node_type)
{
	...
	record = zfs_multiclus_get_group_master(zp->z_zsb->z_os->os_group_name, m_node_type);
	if(record == NULL || record->node_status.status == ZFS_MULTICLUS_NODE_OFFLINE){
		return EPROTO;
	}
	...
	zfs_group_build_header(zp->z_zsb->z_os, msg_header, ZFS_GROUP_CMD_CMD, 
	    wait_flag, op, msg_len, return_len, dst_spa, dst_os, dst_object, 0, 0, 0, 0, 
	    MSG_REQUEST, msg_orig);
	error = zfs_client_send_to_server(zp->z_zsb->z_os, msg_header, (zfs_msg_t *)cmd_msg, B_FALSE);
...
void zfs_client_get_read_data(zfs_msg_t *omsg, zfs_group_header_t *nmsg_header, zfs_msg_t *nmsg)
{
	...
	int *eofp;
	zfs_group_data_read_t *read;
	zfs_group_data_read_t *nread; 
	zfs_group_data_msg_t *data_msg;
	zfs_group_data_msg_t *ndata_msg;
	...
	if (nmsg_header->error == 0) {
		uiomove(nmsg->call.data.data, nread->len, UIO_READ, uiop);
	...
	boolean_t waitting)
{
	...
	if (error == 0){
		...
		*zpp = zfs_znode_alloc_by_group(zsb, 0,
		    &tmp_zp->z_group_id, &tmp_phy);
	}
	iput(ZTOI(tmp_zp));
...
int zfs_client_do_notify_file_info(znode_t* zp, uint64_t dst_spa, uint64_t dst_objset, uint64_t dst_object, uint64_t update_node_info)
{
	...
	return zfs_group_proc_notify(zp->z_zsb->z_os, dst_spa, dst_objset, dst_object, NOTIFY_FILE_INFO, &arg);
}
...
// //	zfsvfs = (zfsvfs_t *)dmu_objset_get_user(os);
...
//	pzp = VTOZ(pvp);
	...
	nrec = kmem_alloc(sizeof (zfs_group_znode_record_t), KM_SLEEP);
//	if ((error = zfs_group_proc_name(VTOZ(pvp), NAME_MKDIR, &mkdir,
...
zfs_client_readdir(struct inode *ip, struct dir_context *ctx, int maxentrynum, cred_t *cr, int flag)
{
	...
	aiov.iov_base = kmem_alloc(sizeof(zfs_group_max_dataseg_size), KM_NOSLEEP);
	...
	auio.uio_fmode = (uint16_t)maxentrynum; 
	...
	for (dp = (struct linux_dirent64 *)aiov.iov_base;
		    (uint64_t)dp < (uint64_t)aiov.iov_base + dbuflen;
		    dp = (struct linux_dirent64 *)((uint64_t)dp + dp->d_reclen)) {
		done = !dir_emit(ctx, dp->d_name, strlen(dp->d_name),
		    dp->d_ino, dp->d_type);
		if (done)
			break;
	}
...
#endif
out:
	kmem_free(aiov.iov_base, sizeof(zfs_group_max_dataseg_size));
	return (error);
}


int	zfs_client_symlink(
	struct inode *pip, char *cp, vattr_t *vap, char *tnm,		
	cred_t *credp, int flag)
{
	...
	error = zfs_group_proc_name(ITOZ(pip), NAME_SYMLINK,
...
zfs_client_readlink(struct inode *ip, uio_t *uio, cred_t *cr, caller_context_t *ct)
{
	int error;
	znode_t *zp = ITOZ(ip);
	error = zfs_client_read_data(zp->z_zsb, zp, uio, uio->uio_resid,
...
zfs_client_read(struct inode *ip, uio_t *uio, int ioflag, cred_t *cr)
{
	...
	while (n > 0) {
		...
		error = zfs_client_read_data(zp->z_zsb, zp, uio, nbytes, DATA_READ,
...
zfs_client_read2(struct inode *ip, uio_t *uio, int ioflag, cred_t *cr)
{
	...
	while (n > 0) {
		...
		error = zfs_client_read_data2(zp->z_zsb, zp, uio, nbytes, DATA_READ,
		    cr, ioflag, DATA_TO_DATA, NULL);
		if (error != 0)
			break;
...
zfs_client_write(struct inode *ip, uio_t *uio, int ioflag, cred_t *cr, caller_context_t *ct)
{
	...
	while (n > 0) {
		nbytes = MIN(n, zfs_group_max_dataseg_size -
		    P2PHASE(uio->uio_loffset, zfs_group_max_dataseg_size));
		error = zfs_client_write_data(zp->z_zsb, zp, uio, nbytes,
		    cr, ioflag);
		if (error != 0)
			break;
...
zfs_client_write2(struct inode *ip, uio_t *uio, int ioflag, cred_t *cr, caller_context_t *ct)
{
	...
	while (n > 0) {
		nbytes = MIN(n, zfs_group_max_dataseg_size -
		    P2PHASE(uio->uio_loffset, zfs_group_max_dataseg_size));
		error = zfs_client_write_data2(zp->z_zsb, zp, uio, nbytes,
		    cr, ioflag);
		if (error != 0)
			break;
...
{	
	...
	stat_arg.return_ptr= (uintptr_t)(&remote_err);
	stat_arg.return_size= sizeof(remote_err);
	err = zfs_proc_dir_low(zp->z_zsb->z_os, SC_DIR_LOW, SHARE_WAIT, &stat_arg,
...
int zfs_group_broadcast_unflag_overquota(znode_t *zp, uint64_t old_dirquota_id)
{
	...
	for (i = 0; i < ZFS_MULTICLUS_GROUP_NODE_NUM; i++) {
		...
		if(record->used && ZFS_MULTICLUS_MASTER != record->node_type) {
			/* process unflag */
			err = zfs_proc_cmd(zp->z_zsb, SC_UNFLAG_OVERQUOTA, SHARE_WAIT, &cmd_arg,
