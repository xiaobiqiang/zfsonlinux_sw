...
static void
qlt_handle_ct_completion(struct scsi_qla_host *vha, uint8_t *rsp)
{
	fct_cmd_t	*cmd;
	fct_sol_ct_t	*ct;
	qlt_cmd_t	*qcmd;
	...
	if (!CMD_HANDLE_VALID(hndl)) {
		ql_dbg(ql_dbg_tgt, vha, 0xe041, "handle = %xh\n", hndl);
		/*
 ...
 */
static void
qlt_handle_unsol_els_completion(struct scsi_qla_host *vha, uint8_t *rsp)
{
	char		info[160];
	fct_cmd_t	*cmd;
	qlt_cmd_t	*qcmd;
	...
	hndl = ioread32(rsp+4);
	status = ioread16(rsp+8);
	subcode1 = ioread32(rsp+0x24);
	subcode2 = ioread32(rsp+0x28);
	elsop = rsp[0x16];
	...
	if (status == 5) {
		 ...
		 */
		ql_dbg(ql_dbg_tgt, vha, 0xe041, "status = %xh\n", status);
	...
	if (status == 0) {
		...
		if ((elsop == ELS_OP_LOGO) &&
		    (vha->hw->current_topology == ISP_CFG_N)) {
			ql_dbg(ql_dbg_tgt, vha, 0xe041, 
				"reset link since this is LOGO and N2N\n");
...
static void
qlt_handle_unsol_els_abort_completion(struct scsi_qla_host *vha, uint8_t *rsp)
{
	char		info[160];
	fct_cmd_t	*cmd;
	qlt_cmd_t	*qcmd;
...
static void
qlt_handle_sol_els_completion(struct scsi_qla_host *vha, uint8_t *rsp)
{
	char		info[160];
	fct_cmd_t	*cmd;
	fct_els_t	*els;
	...
	qcmd = (qlt_cmd_t *)cmd->cmd_fca_private;
	qcmd->fw_xchg_addr = ioread32(&rsp[0x10]);
	...
	struct abort_entry_24xx *pkt)
{
	...
	cmd = fct_handle_to_cmd(vha->qlt_port, h);
	if (cmd == NULL) {
		ql_dbg(ql_dbg_tgt, vha, 0xe041, 
			"fct_handle_to_cmd cmd==NULL, hndl=%xh\n", h);
		/*
...
process_err:
        ...
        case ELS_IOCB_TYPE:
			...
			if (c == 0) {
				...
				qlt_handle_unsol_els_abort_completion(vha,
				    resp);
			} else {
				qlt_handle_unsol_els_completion(vha,
				    resp);
			}
			//qla24xx_els_ct_entry(vha, rsp->req, pkt, CT_IOCB_TYPE);
			break;
