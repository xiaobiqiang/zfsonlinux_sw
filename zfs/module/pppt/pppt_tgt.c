...
static void
pppt_tgt_dereg_retry(unsigned long arg);
...
pppt_tgt_lookup_locked(scsi_devid_desc_t *tgt_devid)
{
	...
	tgt_devid->code_set = CODE_SET_ASCII;
...
pppt_tgt_offline_task(void *arg)
{
	...
	cmn_err(CE_NOTE, "%s complete %p", __func__, (void *)tgt);
}
...
pppt_tgt_dereg_retry(unsigned long arg)
{
	...
	if (taskq_dispatch(pppt_global.global_dispatch_taskq,
		...
		init_timer(&timer);
		timer.expires = jiffies + TGT_DEREG_RETRY_SECONDS * HZ;
		timer.function = pppt_tgt_dereg_retry;
		timer.data = (unsigned long)tgt;
		add_timer(&timer);
	}
}
